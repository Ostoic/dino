#pragma once

#include <string_view>

namespace dino::wow::net::messages
{
	enum class server
	{
		null = 0x0,
		chat_message = 0x96,
		emote = 0x103,
		text_emote = 0x105,
		gm_chat_message = 0x3B3,
		new_world = 0x3E,
		cast_failed = 0x130,

		spell_log_miss = 0x24B,
		spell_log_execute = 0x24C,
		spell_damage_shield = 0x24F,
		spell_non_melee_damage_log = 0x250,
		spell_instakill_log = 0x32F,
		spell_update_chain_targets = 0x330,
		spell_break_log = 0x14F,
		spell_heal_log = 0x150,
		spell_energize_log = 0x151,
		spell_dispell_log = 0x27B,
		spell_steal_log = 0x333,



		health_update = 0x47F,
		power_update = 0x480,
		guild_roster = 0x8A,

		spell_start = 0x131,
		spell_go = 0x132,
		spell_failure = 0x133,
		spell_failed_other = 0x2A6,
		spell_cooldown = 0x134,
		cooldown_event = 0x135,

		notification = 0x1CB,

		unit_spellcast_start = 0x14D,
		stand_state_update = 0x29D,
		loot_list = 0x3F9,

		monster_move = 0xDD,
		ai_reaction = 0x13C,

		mount_result = 0x16E,
		dismount_result = 0x16F,

		cooldown_cheat = 0x1E1,

		combat_log_multiple = 0x514,

		debug_aura_proc = 0x24D,
		periodic_aura_log = 0x24E,
		cancel_combat = 0x14E,
		break_target = 0x152,

		update_object = 0xA9,
		destroy_object = 0xAA,

		durability_damage_death = 0x2BD,

		server_message = 0x291,
		warden_data = 0x2E6,
		zone_under_attack = 0x254,
	};

	enum class client
	{
		chat_message = 0x95,
		emote = 0x102,
		text_emote = 0x104,

		stand_state_change = 0x101,
		warden_data = 0x2E7,
	};

	enum class opcode
	{
		MSG_NULL_ACTION = 0x0,
		CMSG_BOOTME = 0x1,
		CMSG_DBLOOKUP = 0x2,
		SMSG_DBLOOKUP = 0x3,
		CMSG_QUERY_OBJECT_POSITION = 0x4,
		SMSG_QUERY_OBJECT_POSITION = 0x5,
		CMSG_QUERY_OBJECT_ROTATION = 0x6,
		SMSG_QUERY_OBJECT_ROTATION = 0x7,
		CMSG_WORLD_TELEPORT = 0x8,
		CMSG_TELEPORT_TO_UNIT = 0x9,
		CMSG_ZONE_MAP = 0xA,
		SMSG_ZONE_MAP = 0xB,
		CMSG_DEBUG_CHANGECELLZONE = 0xC,
		CMSG_MOVE_CHARACTER_CHEAT = 0xD,
		SMSG_MOVE_CHARACTER_CHEAT = 0xE,
		CMSG_RECHARGE = 0xF,
		CMSG_LEARN_SPELL = 0x10,
		CMSG_CREATEMONSTER = 0x11,
		CMSG_DESTROYMONSTER = 0x12,
		CMSG_CREATEITEM = 0x13,
		CMSG_CREATEGAMEOBJECT = 0x14,
		SMSG_CHECK_FOR_BOTS = 0x15,
		CMSG_MAKEMONSTERATTACKGUID = 0x16,
		CMSG_BOT_DETECTED2 = 0x17,
		CMSG_FORCEACTION = 0x18,
		CMSG_FORCEACTIONONOTHER = 0x19,
		CMSG_FORCEACTIONSHOW = 0x1A,
		SMSG_FORCEACTIONSHOW = 0x1B,
		CMSG_PETGODMODE = 0x1C,
		SMSG_PETGODMODE = 0x1D,
		SMSG_REFER_A_FRIEND_EXPIRED = 0x1E,
		CMSG_WEATHER_SPEED_CHEAT = 0x1F,
		CMSG_UNDRESSPLAYER = 0x20,
		CMSG_BEASTMASTER = 0x21,
		CMSG_GODMODE = 0x22,
		SMSG_GODMODE = 0x23,
		CMSG_CHEAT_SETMONEY = 0x24,
		CMSG_LEVEL_CHEAT = 0x25,
		CMSG_PET_LEVEL_CHEAT = 0x26,
		CMSG_SET_WORLDSTATE = 0x27,
		CMSG_COOLDOWN_CHEAT = 0x28,
		CMSG_USE_SKILL_CHEAT = 0x29,
		CMSG_FLAG_QUEST = 0x2A,
		CMSG_FLAG_QUEST_FINISH = 0x2B,
		CMSG_CLEAR_QUEST = 0x2C,
		CMSG_SEND_EVENT = 0x2D,
		CMSG_DEBUG_AISTATE = 0x2E,
		SMSG_DEBUG_AISTATE = 0x2F,
		CMSG_DISABLE_PVP_CHEAT = 0x30,
		CMSG_ADVANCE_SPAWN_TIME = 0x31,
		SMSG_DESTRUCTIBLE_BUILDING_DAMAGE = 0x32,
		CMSG_AUTH_SRP6_BEGIN = 0x33,
		CMSG_AUTH_SRP6_PROOF = 0x34,
		CMSG_AUTH_SRP6_RECODE = 0x35,
		CMSG_CHAR_CREATE = 0x36,
		CMSG_CHAR_ENUM = 0x37,
		CMSG_CHAR_DELETE = 0x38,
		SMSG_AUTH_SRP6_RESPONSE = 0x39,
		SMSG_CHAR_CREATE = 0x3A,
		SMSG_CHAR_ENUM = 0x3B,
		SMSG_CHAR_DELETE = 0x3C,
		CMSG_PLAYER_LOGIN = 0x3D,
		SMSG_NEW_WORLD = 0x3E,
		SMSG_TRANSFER_PENDING = 0x3F,
		SMSG_TRANSFER_ABORTED = 0x40,
		SMSG_CHARACTER_LOGIN_FAILED = 0x41,
		SMSG_LOGIN_SETTIMESPEED = 0x42,
		SMSG_GAMETIME_UPDATE = 0x43,
		CMSG_GAMETIME_SET = 0x44,
		SMSG_GAMETIME_SET = 0x45,
		CMSG_GAMESPEED_SET = 0x46,
		SMSG_GAMESPEED_SET = 0x47,
		CMSG_SERVERTIME = 0x48,
		SMSG_SERVERTIME = 0x49,
		CMSG_PLAYER_LOGOUT = 0x4A,
		CMSG_LOGOUT_REQUEST = 0x4B,
		SMSG_LOGOUT_RESPONSE = 0x4C,
		SMSG_LOGOUT_COMPLETE = 0x4D,
		CMSG_LOGOUT_CANCEL = 0x4E,
		SMSG_LOGOUT_CANCEL_ACK = 0x4F,
		CMSG_NAME_QUERY = 0x50,
		SMSG_NAME_QUERY_RESPONSE = 0x51,
		CMSG_PET_NAME_QUERY = 0x52,
		SMSG_PET_NAME_QUERY_RESPONSE = 0x53,
		CMSG_GUILD_QUERY = 0x54,
		SMSG_GUILD_QUERY_RESPONSE = 0x55,
		CMSG_ITEM_QUERY_SINGLE = 0x56,
		CMSG_ITEM_QUERY_MULTIPLE = 0x57,
		SMSG_ITEM_QUERY_SINGLE_RESPONSE = 0x58,
		SMSG_ITEM_QUERY_MULTIPLE_RESPONSE = 0x59,
		CMSG_PAGE_TEXT_QUERY = 0x5A,
		SMSG_PAGE_TEXT_QUERY_RESPONSE = 0x5B,
		CMSG_QUEST_QUERY = 0x5C,
		SMSG_QUEST_QUERY_RESPONSE = 0x5D,
		CMSG_GAMEOBJECT_QUERY = 0x5E,
		SMSG_GAMEOBJECT_QUERY_RESPONSE = 0x5F,
		CMSG_CREATURE_QUERY = 0x60,
		SMSG_CREATURE_QUERY_RESPONSE = 0x61,
		CMSG_WHO = 0x62,
		SMSG_WHO = 0x63,
		CMSG_WHOIS = 0x64,
		SMSG_WHOIS = 0x65,
		CMSG_CONTACT_LIST = 0x66,
		SMSG_CONTACT_LIST = 0x67,
		SMSG_FRIEND_STATUS = 0x68,
		CMSG_ADD_FRIEND = 0x69,
		CMSG_DEL_FRIEND = 0x6A,
		CMSG_SET_CONTACT_NOTES = 0x6B,
		CMSG_ADD_IGNORE = 0x6C,
		CMSG_DEL_IGNORE = 0x6D,
		CMSG_GROUP_INVITE = 0x6E,
		SMSG_GROUP_INVITE = 0x6F,
		CMSG_GROUP_CANCEL = 0x70,
		SMSG_GROUP_CANCEL = 0x71,
		CMSG_GROUP_ACCEPT = 0x72,
		CMSG_GROUP_DECLINE = 0x73,
		SMSG_GROUP_DECLINE = 0x74,
		CMSG_GROUP_UNINVITE = 0x75,
		CMSG_GROUP_UNINVITE_GUID = 0x76,
		SMSG_GROUP_UNINVITE = 0x77,
		CMSG_GROUP_SET_LEADER = 0x78,
		SMSG_GROUP_SET_LEADER = 0x79,
		CMSG_LOOT_METHOD = 0x7A,
		CMSG_GROUP_DISBAND = 0x7B,
		SMSG_GROUP_DESTROYED = 0x7C,
		SMSG_GROUP_LIST = 0x7D,
		SMSG_PARTY_MEMBER_STATS = 0x7E,
		SMSG_PARTY_COMMAND_RESULT = 0x7F,
		UMSG_UPDATE_GROUP_MEMBERS = 0x80,
		CMSG_GUILD_CREATE = 0x81,
		CMSG_GUILD_INVITE = 0x82,
		SMSG_GUILD_INVITE = 0x83,
		CMSG_GUILD_ACCEPT = 0x84,
		CMSG_GUILD_DECLINE = 0x85,
		SMSG_GUILD_DECLINE = 0x86,
		CMSG_GUILD_INFO = 0x87,
		SMSG_GUILD_INFO = 0x88,
		CMSG_GUILD_ROSTER = 0x89,
		SMSG_GUILD_ROSTER = 0x8A,
		CMSG_GUILD_PROMOTE = 0x8B,
		CMSG_GUILD_DEMOTE = 0x8C,
		CMSG_GUILD_LEAVE = 0x8D,
		CMSG_GUILD_REMOVE = 0x8E,
		CMSG_GUILD_DISBAND = 0x8F,
		CMSG_GUILD_LEADER = 0x90,
		CMSG_GUILD_MOTD = 0x91,
		SMSG_GUILD_EVENT = 0x92,
		SMSG_GUILD_COMMAND_RESULT = 0x93,
		UMSG_UPDATE_GUILD = 0x94,
		CMSG_MESSAGECHAT = 0x95,
		SMSG_MESSAGECHAT = 0x96,
		CMSG_JOIN_CHANNEL = 0x97,
		CMSG_LEAVE_CHANNEL = 0x98,
		SMSG_CHANNEL_NOTIFY = 0x99,
		CMSG_CHANNEL_LIST = 0x9A,
		SMSG_CHANNEL_LIST = 0x9B,
		CMSG_CHANNEL_PASSWORD = 0x9C,
		CMSG_CHANNEL_SET_OWNER = 0x9D,
		CMSG_CHANNEL_OWNER = 0x9E,
		CMSG_CHANNEL_MODERATOR = 0x9F,
		CMSG_CHANNEL_UNMODERATOR = 0xA0,
		CMSG_CHANNEL_MUTE = 0xA1,
		CMSG_CHANNEL_UNMUTE = 0xA2,
		CMSG_CHANNEL_INVITE = 0xA3,
		CMSG_CHANNEL_KICK = 0xA4,
		CMSG_CHANNEL_BAN = 0xA5,
		CMSG_CHANNEL_UNBAN = 0xA6,
		CMSG_CHANNEL_ANNOUNCEMENTS = 0xA7,
		CMSG_CHANNEL_MODERATE = 0xA8,
		SMSG_UPDATE_OBJECT = 0xA9,
		SMSG_DESTROY_OBJECT = 0xAA,
		CMSG_USE_ITEM = 0xAB,
		CMSG_OPEN_ITEM = 0xAC,
		CMSG_READ_ITEM = 0xAD,
		SMSG_READ_ITEM_OK = 0xAE,
		SMSG_READ_ITEM_FAILED = 0xAF,
		SMSG_ITEM_COOLDOWN = 0xB0,
		CMSG_GAMEOBJ_USE = 0xB1,
		CMSG_DESTROY_ITEMS = 0xB2,
		SMSG_GAMEOBJECT_CUSTOM_ANIM = 0xB3,
		CMSG_AREATRIGGER = 0xB4,
		MSG_MOVE_START_FORWARD = 0xB5,
		MSG_MOVE_START_BACKWARD = 0xB6,
		MSG_MOVE_STOP = 0xB7,
		MSG_MOVE_START_STRAFE_LEFT = 0xB8,
		MSG_MOVE_START_STRAFE_RIGHT = 0xB9,
		MSG_MOVE_STOP_STRAFE = 0xBA,
		MSG_MOVE_JUMP = 0xBB,
		MSG_MOVE_START_TURN_LEFT = 0xBC,
		MSG_MOVE_START_TURN_RIGHT = 0xBD,
		MSG_MOVE_STOP_TURN = 0xBE,
		MSG_MOVE_START_PITCH_UP = 0xBF,
		MSG_MOVE_START_PITCH_DOWN = 0xC0,
		MSG_MOVE_STOP_PITCH = 0xC1,
		MSG_MOVE_SET_RUN_MODE = 0xC2,
		MSG_MOVE_SET_WALK_MODE = 0xC3,
		MSG_MOVE_TOGGLE_LOGGING = 0xC4,
		MSG_MOVE_TELEPORT = 0xC5,
		MSG_MOVE_TELEPORT_CHEAT = 0xC6,
		MSG_MOVE_TELEPORT_ACK = 0xC7,
		MSG_MOVE_TOGGLE_FALL_LOGGING = 0xC8,
		MSG_MOVE_FALL_LAND = 0xC9,
		MSG_MOVE_START_SWIM = 0xCA,
		MSG_MOVE_STOP_SWIM = 0xCB,
		MSG_MOVE_SET_RUN_SPEED_CHEAT = 0xCC,
		MSG_MOVE_SET_RUN_SPEED = 0xCD,
		MSG_MOVE_SET_RUN_BACK_SPEED_CHEAT = 0xCE,
		MSG_MOVE_SET_RUN_BACK_SPEED = 0xCF,
		MSG_MOVE_SET_WALK_SPEED_CHEAT = 0xD0,
		MSG_MOVE_SET_WALK_SPEED = 0xD1,
		MSG_MOVE_SET_SWIM_SPEED_CHEAT = 0xD2,
		MSG_MOVE_SET_SWIM_SPEED = 0xD3,
		MSG_MOVE_SET_SWIM_BACK_SPEED_CHEAT = 0xD4,
		MSG_MOVE_SET_SWIM_BACK_SPEED = 0xD5,
		MSG_MOVE_SET_ALL_SPEED_CHEAT = 0xD6,
		MSG_MOVE_SET_TURN_RATE_CHEAT = 0xD7,
		MSG_MOVE_SET_TURN_RATE = 0xD8,
		MSG_MOVE_TOGGLE_COLLISION_CHEAT = 0xD9,
		MSG_MOVE_SET_FACING = 0xDA,
		MSG_MOVE_SET_PITCH = 0xDB,
		MSG_MOVE_WORLDPORT_ACK = 0xDC,
		SMSG_MONSTER_MOVE = 0xDD,
		SMSG_MOVE_WATER_WALK = 0xDE,
		SMSG_MOVE_LAND_WALK = 0xDF,
		CMSG_MOVE_CHARM_PORT_CHEAT = 0xE0,
		CMSG_MOVE_SET_RAW_POSITION = 0xE1,
		SMSG_FORCE_RUN_SPEED_CHANGE = 0xE2,
		CMSG_FORCE_RUN_SPEED_CHANGE_ACK = 0xE3,
		SMSG_FORCE_RUN_BACK_SPEED_CHANGE = 0xE4,
		CMSG_FORCE_RUN_BACK_SPEED_CHANGE_ACK = 0xE5,
		SMSG_FORCE_SWIM_SPEED_CHANGE = 0xE6,
		CMSG_FORCE_SWIM_SPEED_CHANGE_ACK = 0xE7,
		SMSG_FORCE_MOVE_ROOT = 0xE8,
		CMSG_FORCE_MOVE_ROOT_ACK = 0xE9,
		SMSG_FORCE_MOVE_UNROOT = 0xEA,
		CMSG_FORCE_MOVE_UNROOT_ACK = 0xEB,
		MSG_MOVE_ROOT = 0xEC,
		MSG_MOVE_UNROOT = 0xED,
		MSG_MOVE_HEARTBEAT = 0xEE,
		SMSG_MOVE_KNOCK_BACK = 0xEF,
		CMSG_MOVE_KNOCK_BACK_ACK = 0xF0,
		MSG_MOVE_KNOCK_BACK = 0xF1,
		SMSG_MOVE_FEATHER_FALL = 0xF2,
		SMSG_MOVE_NORMAL_FALL = 0xF3,
		SMSG_MOVE_SET_HOVER = 0xF4,
		SMSG_MOVE_UNSET_HOVER = 0xF5,
		CMSG_MOVE_HOVER_ACK = 0xF6,
		MSG_MOVE_HOVER = 0xF7,
		CMSG_TRIGGER_CINEMATIC_CHEAT = 0xF8,
		CMSG_OPENING_CINEMATIC = 0xF9,
		SMSG_TRIGGER_CINEMATIC = 0xFA,
		CMSG_NEXT_CINEMATIC_CAMERA = 0xFB,
		CMSG_COMPLETE_CINEMATIC = 0xFC,
		SMSG_TUTORIAL_FLAGS = 0xFD,
		CMSG_TUTORIAL_FLAG = 0xFE,
		CMSG_TUTORIAL_CLEAR = 0xFF,
		CMSG_TUTORIAL_RESET = 0x100,
		CMSG_STANDSTATECHANGE = 0x101,
		CMSG_EMOTE = 0x102,
		SMSG_EMOTE = 0x103,
		CMSG_TEXT_EMOTE = 0x104,
		SMSG_TEXT_EMOTE = 0x105,
		CMSG_AUTOEQUIP_GROUND_ITEM = 0x106,
		CMSG_AUTOSTORE_GROUND_ITEM = 0x107,
		CMSG_AUTOSTORE_LOOT_ITEM = 0x108,
		CMSG_STORE_LOOT_IN_SLOT = 0x109,
		CMSG_AUTOEQUIP_ITEM = 0x10A,
		CMSG_AUTOSTORE_BAG_ITEM = 0x10B,
		CMSG_SWAP_ITEM = 0x10C,
		CMSG_SWAP_INV_ITEM = 0x10D,
		CMSG_SPLIT_ITEM = 0x10E,
		CMSG_AUTOEQUIP_ITEM_SLOT = 0x10F,
		CMSG_UNCLAIM_LICENSE = 0x110,
		CMSG_DESTROYITEM = 0x111,
		SMSG_INVENTORY_CHANGE_FAILURE = 0x112,
		SMSG_OPEN_CONTAINER = 0x113,
		CMSG_INSPECT = 0x114,
		SMSG_INSPECT_RESULTS_UPDATE = 0x115,
		CMSG_INITIATE_TRADE = 0x116,
		CMSG_BEGIN_TRADE = 0x117,
		CMSG_BUSY_TRADE = 0x118,
		CMSG_IGNORE_TRADE = 0x119,
		CMSG_ACCEPT_TRADE = 0x11A,
		CMSG_UNACCEPT_TRADE = 0x11B,
		CMSG_CANCEL_TRADE = 0x11C,
		CMSG_SET_TRADE_ITEM = 0x11D,
		CMSG_CLEAR_TRADE_ITEM = 0x11E,
		CMSG_SET_TRADE_GOLD = 0x11F,
		SMSG_TRADE_STATUS = 0x120,
		SMSG_TRADE_STATUS_EXTENDED = 0x121,
		SMSG_INITIALIZE_FACTIONS = 0x122,
		SMSG_SET_FACTION_VISIBLE = 0x123,
		SMSG_SET_FACTION_STANDING = 0x124,
		CMSG_SET_FACTION_ATWAR = 0x125,
		CMSG_SET_FACTION_CHEAT = 0x126,
		SMSG_SET_PROFICIENCY = 0x127,
		CMSG_SET_ACTION_BUTTON = 0x128,
		SMSG_ACTION_BUTTONS = 0x129,
		SMSG_INITIAL_SPELLS = 0x12A,
		SMSG_LEARNED_SPELL = 0x12B,
		SMSG_SUPERCEDED_SPELL = 0x12C,
		CMSG_NEW_SPELL_SLOT = 0x12D,
		CMSG_CAST_SPELL = 0x12E,
		CMSG_CANCEL_CAST = 0x12F,
		SMSG_CAST_FAILED = 0x130,
		SMSG_SPELL_START = 0x131,
		SMSG_SPELL_GO = 0x132,
		SMSG_SPELL_FAILURE = 0x133,
		SMSG_SPELL_COOLDOWN = 0x134,
		SMSG_COOLDOWN_EVENT = 0x135,
		CMSG_CANCEL_AURA = 0x136,
		SMSG_EQUIPMENT_SET_ID = 0x137,
		SMSG_PET_CAST_FAILED = 0x138,
		MSG_CHANNEL_START = 0x139,
		MSG_CHANNEL_UPDATE = 0x13A,
		CMSG_CANCEL_CHANNELLING = 0x13B,
		SMSG_AI_REACTION = 0x13C,
		CMSG_SET_SELECTION = 0x13D,
		CMSG_DELETEEQUIPMENT_SET = 0x13E,
		CMSG_INSTANCE_LOCK_RESPONSE = 0x13F,
		CMSG_DEBUG_PASSIVE_AURA = 0x140,
		CMSG_ATTACKSWING = 0x141,
		CMSG_ATTACKSTOP = 0x142,
		SMSG_ATTACKSTART = 0x143,
		SMSG_ATTACKSTOP = 0x144,
		SMSG_ATTACKSWING_NOTINRANGE = 0x145,
		SMSG_ATTACKSWING_BADFACING = 0x146,
		SMSG_PENDING_RAID_LOCK = 0x147,
		SMSG_ATTACKSWING_DEADTARGET = 0x148,
		SMSG_ATTACKSWING_CANT_ATTACK = 0x149,
		SMSG_ATTACKERSTATEUPDATE = 0x14A,
		SMSG_BATTLEFIELD_PORT_DENIED = 0x14B,
		CMSG_PERFORM_ACTION_SET = 0x14C,
		SMSG_RESUME_CAST_BAR = 0x14D,
		SMSG_CANCEL_COMBAT = 0x14E,
		SMSG_SPELLBREAKLOG = 0x14F,
		SMSG_SPELLHEALLOG = 0x150,
		SMSG_SPELLENERGIZELOG = 0x151,
		SMSG_BREAK_TARGET = 0x152,
		CMSG_SAVE_PLAYER = 0x153,
		CMSG_SETDEATHBINDPOINT = 0x154,
		SMSG_BINDPOINTUPDATE = 0x155,
		CMSG_GETDEATHBINDZONE = 0x156,
		SMSG_BINDZONEREPLY = 0x157,
		SMSG_PLAYERBOUND = 0x158,
		SMSG_CLIENT_CONTROL_UPDATE = 0x159,
		CMSG_REPOP_REQUEST = 0x15A,
		SMSG_RESURRECT_REQUEST = 0x15B,
		CMSG_RESURRECT_RESPONSE = 0x15C,
		CMSG_LOOT = 0x15D,
		CMSG_LOOT_MONEY = 0x15E,
		CMSG_LOOT_RELEASE = 0x15F,
		SMSG_LOOT_RESPONSE = 0x160,
		SMSG_LOOT_RELEASE_RESPONSE = 0x161,
		SMSG_LOOT_REMOVED = 0x162,
		SMSG_LOOT_MONEY_NOTIFY = 0x163,
		SMSG_LOOT_ITEM_NOTIFY = 0x164,
		SMSG_LOOT_CLEAR_MONEY = 0x165,
		SMSG_ITEM_PUSH_RESULT = 0x166,
		SMSG_DUEL_REQUESTED = 0x167,
		SMSG_DUEL_OUTOFBOUNDS = 0x168,
		SMSG_DUEL_INBOUNDS = 0x169,
		SMSG_DUEL_COMPLETE = 0x16A,
		SMSG_DUEL_WINNER = 0x16B,
		CMSG_DUEL_ACCEPTED = 0x16C,
		CMSG_DUEL_CANCELLED = 0x16D,
		SMSG_MOUNTRESULT = 0x16E,
		SMSG_DISMOUNTRESULT = 0x16F,
		SMSG_REMOVED_FROM_PVP_QUEUE = 0x170,
		CMSG_MOUNTSPECIAL_ANIM = 0x171,
		SMSG_MOUNTSPECIAL_ANIM = 0x172,
		SMSG_PET_TAME_FAILURE = 0x173,
		CMSG_PET_SET_ACTION = 0x174,
		CMSG_PET_ACTION = 0x175,
		CMSG_PET_ABANDON = 0x176,
		CMSG_PET_RENAME = 0x177,
		SMSG_PET_NAME_INVALID = 0x178,
		SMSG_PET_SPELLS = 0x179,
		SMSG_PET_MODE = 0x17A,
		CMSG_GOSSIP_HELLO = 0x17B,
		CMSG_GOSSIP_SELECT_OPTION = 0x17C,
		SMSG_GOSSIP_MESSAGE = 0x17D,
		SMSG_GOSSIP_COMPLETE = 0x17E,
		CMSG_NPC_TEXT_QUERY = 0x17F,
		SMSG_NPC_TEXT_UPDATE = 0x180,
		SMSG_NPC_WONT_TALK = 0x181,
		CMSG_QUESTGIVER_STATUS_QUERY = 0x182,
		SMSG_QUESTGIVER_STATUS = 0x183,
		CMSG_QUESTGIVER_HELLO = 0x184,
		SMSG_QUESTGIVER_QUEST_LIST = 0x185,
		CMSG_QUESTGIVER_QUERY_QUEST = 0x186,
		CMSG_QUESTGIVER_QUEST_AUTOLAUNCH = 0x187,
		SMSG_QUESTGIVER_QUEST_DETAILS = 0x188,
		CMSG_QUESTGIVER_ACCEPT_QUEST = 0x189,
		CMSG_QUESTGIVER_COMPLETE_QUEST = 0x18A,
		SMSG_QUESTGIVER_REQUEST_ITEMS = 0x18B,
		CMSG_QUESTGIVER_REQUEST_REWARD = 0x18C,
		SMSG_QUESTGIVER_OFFER_REWARD = 0x18D,
		CMSG_QUESTGIVER_CHOOSE_REWARD = 0x18E,
		SMSG_QUESTGIVER_QUEST_INVALID = 0x18F,
		CMSG_QUESTGIVER_CANCEL = 0x190,
		SMSG_QUESTGIVER_QUEST_COMPLETE = 0x191,
		SMSG_QUESTGIVER_QUEST_FAILED = 0x192,
		CMSG_QUESTLOG_SWAP_QUEST = 0x193,
		CMSG_QUESTLOG_REMOVE_QUEST = 0x194,
		SMSG_QUESTLOG_FULL = 0x195,
		SMSG_QUESTUPDATE_FAILED = 0x196,
		SMSG_QUESTUPDATE_FAILEDTIMER = 0x197,
		SMSG_QUESTUPDATE_COMPLETE = 0x198,
		SMSG_QUESTUPDATE_ADD_KILL = 0x199,
		SMSG_QUESTUPDATE_ADD_ITEM_OBOSLETE = 0x19A,
		CMSG_QUEST_CONFIRM_ACCEPT = 0x19B,
		SMSG_QUEST_CONFIRM_ACCEPT = 0x19C,
		CMSG_PUSHQUESTTOPARTY = 0x19D,
		CMSG_LIST_INVENTORY = 0x19E,
		SMSG_LIST_INVENTORY = 0x19F,
		CMSG_SELL_ITEM = 0x1A0,
		SMSG_SELL_ITEM = 0x1A1,
		CMSG_BUY_ITEM = 0x1A2,
		CMSG_BUY_ITEM_IN_SLOT = 0x1A3,
		SMSG_BUY_ITEM = 0x1A4,
		SMSG_BUY_FAILED = 0x1A5,
		CMSG_TAXICLEARALLNODES = 0x1A6,
		CMSG_TAXIENABLEALLNODES = 0x1A7,
		CMSG_TAXISHOWNODES = 0x1A8,
		SMSG_SHOWTAXINODES = 0x1A9,
		CMSG_TAXINODE_STATUS_QUERY = 0x1AA,
		SMSG_TAXINODE_STATUS = 0x1AB,
		CMSG_TAXIQUERYAVAILABLENODES = 0x1AC,
		CMSG_ACTIVATETAXI = 0x1AD,
		SMSG_ACTIVATETAXIREPLY = 0x1AE,
		SMSG_NEW_TAXI_PATH = 0x1AF,
		CMSG_TRAINER_LIST = 0x1B0,
		SMSG_TRAINER_LIST = 0x1B1,
		CMSG_TRAINER_BUY_SPELL = 0x1B2,
		SMSG_TRAINER_BUY_SUCCEEDED = 0x1B3,
		SMSG_TRAINER_BUY_FAILED = 0x1B4,
		CMSG_BINDER_ACTIVATE = 0x1B5,
		SMSG_PLAYERBINDERROR = 0x1B6,
		CMSG_BANKER_ACTIVATE = 0x1B7,
		SMSG_SHOW_BANK = 0x1B8,
		CMSG_BUY_BANK_SLOT = 0x1B9,
		SMSG_BUY_BANK_SLOT_RESULT = 0x1BA,
		CMSG_PETITION_SHOWLIST = 0x1BB,
		SMSG_PETITION_SHOWLIST = 0x1BC,
		CMSG_PETITION_BUY = 0x1BD,
		CMSG_PETITION_SHOW_SIGNATURES = 0x1BE,
		SMSG_PETITION_SHOW_SIGNATURES = 0x1BF,
		CMSG_PETITION_SIGN = 0x1C0,
		SMSG_PETITION_SIGN_RESULTS = 0x1C1,
		MSG_PETITION_DECLINE = 0x1C2,
		CMSG_OFFER_PETITION = 0x1C3,
		CMSG_TURN_IN_PETITION = 0x1C4,
		SMSG_TURN_IN_PETITION_RESULTS = 0x1C5,
		CMSG_PETITION_QUERY = 0x1C6,
		SMSG_PETITION_QUERY_RESPONSE = 0x1C7,
		SMSG_FISH_NOT_HOOKED = 0x1C8,
		SMSG_FISH_ESCAPED = 0x1C9,
		CMSG_BUG = 0x1CA,
		SMSG_NOTIFICATION = 0x1CB,
		CMSG_PLAYED_TIME = 0x1CC,
		SMSG_PLAYED_TIME = 0x1CD,
		CMSG_QUERY_TIME = 0x1CE,
		SMSG_QUERY_TIME_RESPONSE = 0x1CF,
		SMSG_LOG_XPGAIN = 0x1D0,
		SMSG_AURACASTLOG = 0x1D1,
		CMSG_RECLAIM_CORPSE = 0x1D2,
		CMSG_WRAP_ITEM = 0x1D3,
		SMSG_LEVELUP_INFO = 0x1D4,
		MSG_MINIMAP_PING = 0x1D5,
		SMSG_RESISTLOG = 0x1D6,
		SMSG_ENCHANTMENTLOG = 0x1D7,
		CMSG_SET_SKILL_CHEAT = 0x1D8,
		SMSG_START_MIRROR_TIMER = 0x1D9,
		SMSG_PAUSE_MIRROR_TIMER = 0x1DA,
		SMSG_STOP_MIRROR_TIMER = 0x1DB,
		CMSG_PING = 0x1DC,
		SMSG_PONG = 0x1DD,
		SMSG_CLEAR_COOLDOWN = 0x1DE,
		SMSG_GAMEOBJECT_PAGETEXT = 0x1DF,
		CMSG_SETSHEATHED = 0x1E0,
		SMSG_COOLDOWN_CHEAT = 0x1E1,
		SMSG_SPELL_DELAYED = 0x1E2,
		CMSG_QUEST_POI_QUERY = 0x1E3,
		SMSG_QUEST_POI_QUERY_RESPONSE = 0x1E4,
		CMSG_GHOST = 0x1E5,
		CMSG_GM_INVIS = 0x1E6,
		SMSG_INVALID_PROMOTION_CODE = 0x1E7,
		MSG_GM_BIND_OTHER = 0x1E8,
		MSG_GM_SUMMON = 0x1E9,
		SMSG_ITEM_TIME_UPDATE = 0x1EA,
		SMSG_ITEM_ENCHANT_TIME_UPDATE = 0x1EB,
		SMSG_AUTH_CHALLENGE = 0x1EC,
		CMSG_AUTH_SESSION = 0x1ED,
		SMSG_AUTH_RESPONSE = 0x1EE,
		MSG_GM_SHOWLABEL = 0x1EF,
		CMSG_PET_CAST_SPELL = 0x1F0,
		MSG_SAVE_GUILD_EMBLEM = 0x1F1,
		MSG_TABARDVENDOR_ACTIVATE = 0x1F2,
		SMSG_PLAY_SPELL_VISUAL = 0x1F3,
		CMSG_ZONEUPDATE = 0x1F4,
		SMSG_PARTYKILLLOG = 0x1F5,
		SMSG_COMPRESSED_UPDATE_OBJECT = 0x1F6,
		SMSG_PLAY_SPELL_IMPACT = 0x1F7,
		SMSG_EXPLORATION_EXPERIENCE = 0x1F8,
		CMSG_GM_SET_SECURITY_GROUP = 0x1F9,
		CMSG_GM_NUKE = 0x1FA,
		MSG_RANDOM_ROLL = 0x1FB,
		SMSG_ENVIRONMENTALDAMAGELOG = 0x1FC,
		CMSG_CHANGEPLAYER_DIFFICULTY = 0x1FD,
		SMSG_RWHOIS = 0x1FE,
		SMSG_LFG_PLAYER_REWARD = 0x1FF,
		SMSG_LFG_TELEPORT_DENIED = 0x200,
		CMSG_UNLEARN_SPELL = 0x201,
		CMSG_UNLEARN_SKILL = 0x202,
		SMSG_REMOVED_SPELL = 0x203,
		CMSG_DECHARGE = 0x204,
		CMSG_GMTICKET_CREATE = 0x205,
		SMSG_GMTICKET_CREATE = 0x206,
		CMSG_GMTICKET_UPDATETEXT = 0x207,
		SMSG_GMTICKET_UPDATETEXT = 0x208,
		SMSG_ACCOUNT_DATA_TIMES = 0x209,
		CMSG_REQUEST_ACCOUNT_DATA = 0x20A,
		CMSG_UPDATE_ACCOUNT_DATA = 0x20B,
		SMSG_UPDATE_ACCOUNT_DATA = 0x20C,
		SMSG_CLEAR_FAR_SIGHT_IMMEDIATE = 0x20D,
		SMSG_CHANGEPLAYER_DIFFICULTY_RESULT = 0x20E,
		CMSG_GM_TEACH = 0x20F,
		CMSG_GM_CREATE_ITEM_TARGET = 0x210,
		CMSG_GMTICKET_GETTICKET = 0x211,
		SMSG_GMTICKET_GETTICKET = 0x212,
		CMSG_UNLEARN_TALENTS = 0x213,
		SMSG_INSTANCE_ENCOUNTER = 0x214,
		SMSG_GAMEOBJECT_DESPAWN_ANIM = 0x215,
		MSG_CORPSE_QUERY = 0x216,
		CMSG_GMTICKET_DELETETICKET = 0x217,
		SMSG_GMTICKET_DELETETICKET = 0x218,
		SMSG_CHAT_WRONG_FACTION = 0x219,
		CMSG_GMTICKET_SYSTEMSTATUS = 0x21A,
		SMSG_GMTICKET_SYSTEMSTATUS = 0x21B,
		CMSG_SPIRIT_HEALER_ACTIVATE = 0x21C,
		CMSG_SET_STAT_CHEAT = 0x21D,
		SMSG_QUEST_FORCE_REMOVED = 0x21E,
		CMSG_SKILL_BUY_STEP = 0x21F,
		CMSG_SKILL_BUY_RANK = 0x220,
		CMSG_XP_CHEAT = 0x221,
		SMSG_SPIRIT_HEALER_CONFIRM = 0x222,
		CMSG_CHARACTER_POINT_CHEAT = 0x223,
		SMSG_GOSSIP_POI = 0x224,
		CMSG_CHAT_IGNORED = 0x225,
		CMSG_GM_VISION = 0x226,
		CMSG_SERVER_COMMAND = 0x227,
		CMSG_GM_SILENCE = 0x228,
		CMSG_GM_REVEALTO = 0x229,
		CMSG_GM_RESURRECT = 0x22A,
		CMSG_GM_SUMMONMOB = 0x22B,
		CMSG_GM_MOVECORPSE = 0x22C,
		CMSG_GM_FREEZE = 0x22D,
		CMSG_GM_UBERINVIS = 0x22E,
		CMSG_GM_REQUEST_PLAYER_INFO = 0x22F,
		SMSG_GM_PLAYER_INFO = 0x230,
		CMSG_GUILD_RANK = 0x231,
		CMSG_GUILD_ADD_RANK = 0x232,
		CMSG_GUILD_DEL_RANK = 0x233,
		CMSG_GUILD_SET_PUBLIC_NOTE = 0x234,
		CMSG_GUILD_SET_OFFICER_NOTE = 0x235,
		SMSG_LOGIN_VERIFY_WORLD = 0x236,
		CMSG_CLEAR_EXPLORATION = 0x237,
		CMSG_SEND_MAIL = 0x238,
		SMSG_SEND_MAIL_RESULT = 0x239,
		CMSG_GET_MAIL_LIST = 0x23A,
		SMSG_MAIL_LIST_RESULT = 0x23B,
		CMSG_BATTLEFIELD_LIST = 0x23C,
		SMSG_BATTLEFIELD_LIST = 0x23D,
		CMSG_BATTLEFIELD_JOIN = 0x23E,
		SMSG_FORCE_SET_VEHICLE_REC_ID = 0x23F,
		CMSG_SET_VEHICLE_REC_ID_ACK = 0x240,
		CMSG_TAXICLEARNODE = 0x241,
		CMSG_TAXIENABLENODE = 0x242,
		CMSG_ITEM_TEXT_QUERY = 0x243,
		SMSG_ITEM_TEXT_QUERY_RESPONSE = 0x244,
		CMSG_MAIL_TAKE_MONEY = 0x245,
		CMSG_MAIL_TAKE_ITEM = 0x246,
		CMSG_MAIL_MARK_AS_READ = 0x247,
		CMSG_MAIL_RETURN_TO_SENDER = 0x248,
		CMSG_MAIL_DELETE = 0x249,
		CMSG_MAIL_CREATE_TEXT_ITEM = 0x24A,
		SMSG_SPELLLOGMISS = 0x24B,
		SMSG_SPELLLOGEXECUTE = 0x24C,
		SMSG_DEBUGAURAPROC = 0x24D,
		SMSG_PERIODICAURALOG = 0x24E,
		SMSG_SPELLDAMAGESHIELD = 0x24F,
		SMSG_SPELLNONMELEEDAMAGELOG = 0x250,
		CMSG_LEARN_TALENT = 0x251,
		SMSG_RESURRECT_FAILED = 0x252,
		CMSG_TOGGLE_PVP = 0x253,
		SMSG_ZONE_UNDER_ATTACK = 0x254,
		MSG_AUCTION_HELLO = 0x255,
		CMSG_AUCTION_SELL_ITEM = 0x256,
		CMSG_AUCTION_REMOVE_ITEM = 0x257,
		CMSG_AUCTION_LIST_ITEMS = 0x258,
		CMSG_AUCTION_LIST_OWNER_ITEMS = 0x259,
		CMSG_AUCTION_PLACE_BID = 0x25A,
		SMSG_AUCTION_COMMAND_RESULT = 0x25B,
		SMSG_AUCTION_LIST_RESULT = 0x25C,
		SMSG_AUCTION_OWNER_LIST_RESULT = 0x25D,
		SMSG_AUCTION_BIDDER_NOTIFICATION = 0x25E,
		SMSG_AUCTION_OWNER_NOTIFICATION = 0x25F,
		SMSG_PROCRESIST = 0x260,
		SMSG_COMBAT_EVENT_FAILED = 0x261,
		SMSG_DISPEL_FAILED = 0x262,
		SMSG_SPELLORDAMAGE_IMMUNE = 0x263,
		CMSG_AUCTION_LIST_BIDDER_ITEMS = 0x264,
		SMSG_AUCTION_BIDDER_LIST_RESULT = 0x265,
		SMSG_SET_FLAT_SPELL_MODIFIER = 0x266,
		SMSG_SET_PCT_SPELL_MODIFIER = 0x267,
		CMSG_SET_AMMO = 0x268,
		SMSG_CORPSE_RECLAIM_DELAY = 0x269,
		CMSG_SET_ACTIVE_MOVER = 0x26A,
		CMSG_PET_CANCEL_AURA = 0x26B,
		CMSG_PLAYER_AI_CHEAT = 0x26C,
		CMSG_CANCEL_AUTO_REPEAT_SPELL = 0x26D,
		MSG_GM_ACCOUNT_ONLINE = 0x26E,
		MSG_LIST_STABLED_PETS = 0x26F,
		CMSG_STABLE_PET = 0x270,
		CMSG_UNSTABLE_PET = 0x271,
		CMSG_BUY_STABLE_SLOT = 0x272,
		SMSG_STABLE_RESULT = 0x273,
		CMSG_STABLE_REVIVE_PET = 0x274,
		CMSG_STABLE_SWAP_PET = 0x275,
		MSG_QUEST_PUSH_RESULT = 0x276,
		SMSG_PLAY_MUSIC = 0x277,
		SMSG_PLAY_OBJECT_SOUND = 0x278,
		CMSG_REQUEST_PET_INFO = 0x279,
		CMSG_FAR_SIGHT = 0x27A,
		SMSG_SPELLDISPELLOG = 0x27B,
		SMSG_DAMAGE_CALC_LOG = 0x27C,
		CMSG_ENABLE_DAMAGE_LOG = 0x27D,
		CMSG_GROUP_CHANGE_SUB_GROUP = 0x27E,
		CMSG_REQUEST_PARTY_MEMBER_STATS = 0x27F,
		CMSG_GROUP_SWAP_SUB_GROUP = 0x280,
		CMSG_RESET_FACTION_CHEAT = 0x281,
		CMSG_AUTOSTORE_BANK_ITEM = 0x282,
		CMSG_AUTOBANK_ITEM = 0x283,
		MSG_QUERY_NEXT_MAIL_TIME = 0x284,
		SMSG_RECEIVED_MAIL = 0x285,
		SMSG_RAID_GROUP_ONLY = 0x286,
		CMSG_SET_DURABILITY_CHEAT = 0x287,
		CMSG_SET_PVP_RANK_CHEAT = 0x288,
		CMSG_ADD_PVP_MEDAL_CHEAT = 0x289,
		CMSG_DEL_PVP_MEDAL_CHEAT = 0x28A,
		CMSG_SET_PVP_TITLE = 0x28B,
		SMSG_PVP_CREDIT = 0x28C,
		SMSG_AUCTION_REMOVED_NOTIFICATION = 0x28D,
		CMSG_GROUP_RAID_CONVERT = 0x28E,
		CMSG_GROUP_ASSISTANT_LEADER = 0x28F,
		CMSG_BUYBACK_ITEM = 0x290,
		SMSG_SERVER_MESSAGE = 0x291,
		CMSG_SET_SAVED_INSTANCE_EXTEND = 0x292,
		SMSG_LFG_OFFER_CONTINUE = 0x293,
		CMSG_TEST_DROP_RATE = 0x294,
		SMSG_TEST_DROP_RATE_RESULT = 0x295,
		CMSG_LFG_GET_STATUS = 0x296,
		SMSG_SHOW_MAILBOX = 0x297,
		SMSG_RESET_RANGED_COMBAT_TIMER = 0x298,
		SMSG_CHAT_NOT_IN_PARTY = 0x299,
		CMSG_GMTICKETSYSTEM_TOGGLE = 0x29A,
		CMSG_CANCEL_GROWTH_AURA = 0x29B,
		SMSG_CANCEL_AUTO_REPEAT = 0x29C,
		SMSG_STANDSTATE_UPDATE = 0x29D,
		SMSG_LOOT_ALL_PASSED = 0x29E,
		SMSG_LOOT_ROLL_WON = 0x29F,
		CMSG_LOOT_ROLL = 0x2A0,
		SMSG_LOOT_START_ROLL = 0x2A1,
		SMSG_LOOT_ROLL = 0x2A2,
		CMSG_LOOT_MASTER_GIVE = 0x2A3,
		SMSG_LOOT_MASTER_LIST = 0x2A4,
		SMSG_SET_FORCED_REACTIONS = 0x2A5,
		SMSG_SPELL_FAILED_OTHER = 0x2A6,
		SMSG_GAMEOBJECT_RESET_STATE = 0x2A7,
		CMSG_REPAIR_ITEM = 0x2A8,
		SMSG_CHAT_PLAYER_NOT_FOUND = 0x2A9,
		MSG_TALENT_WIPE_CONFIRM = 0x2AA,
		SMSG_SUMMON_REQUEST = 0x2AB,
		CMSG_SUMMON_RESPONSE = 0x2AC,
		MSG_DEV_SHOWLABEL = 0x2AD,
		SMSG_MONSTER_MOVE_TRANSPORT = 0x2AE,
		SMSG_PET_BROKEN = 0x2AF,
		MSG_MOVE_FEATHER_FALL = 0x2B0,
		MSG_MOVE_WATER_WALK = 0x2B1,
		CMSG_SERVER_BROADCAST = 0x2B2,
		CMSG_SELF_RES = 0x2B3,
		SMSG_FEIGN_DEATH_RESISTED = 0x2B4,
		CMSG_RUN_SCRIPT = 0x2B5,
		SMSG_SCRIPT_MESSAGE = 0x2B6,
		SMSG_DUEL_COUNTDOWN = 0x2B7,
		SMSG_AREA_TRIGGER_MESSAGE = 0x2B8,
		CMSG_SHOWING_HELM = 0x2B9,
		CMSG_SHOWING_CLOAK = 0x2BA,
		SMSG_ROLE_CHOSEN = 0x2BB,
		SMSG_PLAYER_SKINNED = 0x2BC,
		SMSG_DURABILITY_DAMAGE_DEATH = 0x2BD,
		CMSG_SET_EXPLORATION = 0x2BE,
		CMSG_SET_ACTIONBAR_TOGGLES = 0x2BF,
		UMSG_DELETE_GUILD_CHARTER = 0x2C0,
		MSG_PETITION_RENAME = 0x2C1,
		SMSG_INIT_WORLD_STATES = 0x2C2,
		SMSG_UPDATE_WORLD_STATE = 0x2C3,
		CMSG_ITEM_NAME_QUERY = 0x2C4,
		SMSG_ITEM_NAME_QUERY_RESPONSE = 0x2C5,
		SMSG_PET_ACTION_FEEDBACK = 0x2C6,
		CMSG_CHAR_RENAME = 0x2C7,
		SMSG_CHAR_RENAME = 0x2C8,
		CMSG_MOVE_SPLINE_DONE = 0x2C9,
		CMSG_MOVE_FALL_RESET = 0x2CA,
		SMSG_INSTANCE_SAVE_CREATED = 0x2CB,
		SMSG_RAID_INSTANCE_INFO = 0x2CC,
		CMSG_REQUEST_RAID_INFO = 0x2CD,
		CMSG_MOVE_TIME_SKIPPED = 0x2CE,
		CMSG_MOVE_FEATHER_FALL_ACK = 0x2CF,
		CMSG_MOVE_WATER_WALK_ACK = 0x2D0,
		CMSG_MOVE_NOT_ACTIVE_MOVER = 0x2D1,
		SMSG_PLAY_SOUND = 0x2D2,
		CMSG_BATTLEFIELD_STATUS = 0x2D3,
		SMSG_BATTLEFIELD_STATUS = 0x2D4,
		CMSG_BATTLEFIELD_PORT = 0x2D5,
		MSG_INSPECT_HONOR_STATS = 0x2D6,
		CMSG_BATTLEMASTER_HELLO = 0x2D7,
		CMSG_MOVE_START_SWIM_CHEAT = 0x2D8,
		CMSG_MOVE_STOP_SWIM_CHEAT = 0x2D9,
		SMSG_FORCE_WALK_SPEED_CHANGE = 0x2DA,
		CMSG_FORCE_WALK_SPEED_CHANGE_ACK = 0x2DB,
		SMSG_FORCE_SWIM_BACK_SPEED_CHANGE = 0x2DC,
		CMSG_FORCE_SWIM_BACK_SPEED_CHANGE_ACK = 0x2DD,
		SMSG_FORCE_TURN_RATE_CHANGE = 0x2DE,
		CMSG_FORCE_TURN_RATE_CHANGE_ACK = 0x2DF,
		MSG_PVP_LOG_DATA = 0x2E0,
		CMSG_LEAVE_BATTLEFIELD = 0x2E1,
		CMSG_AREA_SPIRIT_HEALER_QUERY = 0x2E2,
		CMSG_AREA_SPIRIT_HEALER_QUEUE = 0x2E3,
		SMSG_AREA_SPIRIT_HEALER_TIME = 0x2E4,
		CMSG_GM_UNTEACH = 0x2E5,
		SMSG_WARDEN_DATA = 0x2E6,
		CMSG_WARDEN_DATA = 0x2E7,
		SMSG_GROUP_JOINED_BATTLEGROUND = 0x2E8,
		MSG_BATTLEGROUND_PLAYER_POSITIONS = 0x2E9,
		CMSG_PET_STOP_ATTACK = 0x2EA,
		SMSG_BINDER_CONFIRM = 0x2EB,
		SMSG_BATTLEGROUND_PLAYER_JOINED = 0x2EC,
		SMSG_BATTLEGROUND_PLAYER_LEFT = 0x2ED,
		CMSG_BATTLEMASTER_JOIN = 0x2EE,
		SMSG_ADDON_INFO = 0x2EF,
		CMSG_PET_UNLEARN = 0x2F0,
		SMSG_PET_UNLEARN_CONFIRM = 0x2F1,
		SMSG_PARTY_MEMBER_STATS_FULL = 0x2F2,
		CMSG_PET_SPELL_AUTOCAST = 0x2F3,
		SMSG_WEATHER = 0x2F4,
		SMSG_PLAY_TIME_WARNING = 0x2F5,
		SMSG_MINIGAME_SETUP = 0x2F6,
		SMSG_MINIGAME_STATE = 0x2F7,
		CMSG_MINIGAME_MOVE = 0x2F8,
		SMSG_MINIGAME_MOVE_FAILED = 0x2F9,
		SMSG_RAID_INSTANCE_MESSAGE = 0x2FA,
		SMSG_COMPRESSED_MOVES = 0x2FB,
		CMSG_GUILD_INFO_TEXT = 0x2FC,
		SMSG_CHAT_RESTRICTED = 0x2FD,
		SMSG_SPLINE_SET_RUN_SPEED = 0x2FE,
		SMSG_SPLINE_SET_RUN_BACK_SPEED = 0x2FF,
		SMSG_SPLINE_SET_SWIM_SPEED = 0x300,
		SMSG_SPLINE_SET_WALK_SPEED = 0x301,
		SMSG_SPLINE_SET_SWIM_BACK_SPEED = 0x302,
		SMSG_SPLINE_SET_TURN_RATE = 0x303,
		SMSG_SPLINE_MOVE_UNROOT = 0x304,
		SMSG_SPLINE_MOVE_FEATHER_FALL = 0x305,
		SMSG_SPLINE_MOVE_NORMAL_FALL = 0x306,
		SMSG_SPLINE_MOVE_SET_HOVER = 0x307,
		SMSG_SPLINE_MOVE_UNSET_HOVER = 0x308,
		SMSG_SPLINE_MOVE_WATER_WALK = 0x309,
		SMSG_SPLINE_MOVE_LAND_WALK = 0x30A,
		SMSG_SPLINE_MOVE_START_SWIM = 0x30B,
		SMSG_SPLINE_MOVE_STOP_SWIM = 0x30C,
		SMSG_SPLINE_MOVE_SET_RUN_MODE = 0x30D,
		SMSG_SPLINE_MOVE_SET_WALK_MODE = 0x30E,
		CMSG_GM_NUKE_ACCOUNT = 0x30F,
		MSG_GM_DESTROY_CORPSE = 0x310,
		CMSG_GM_DESTROY_ONLINE_CORPSE = 0x311,
		CMSG_ACTIVATETAXIEXPRESS = 0x312,
		SMSG_SET_FACTION_ATWAR = 0x313,
		SMSG_GAMETIMEBIAS_SET = 0x314,
		CMSG_DEBUG_ACTIONS_START = 0x315,
		CMSG_DEBUG_ACTIONS_STOP = 0x316,
		CMSG_SET_FACTION_INACTIVE = 0x317,
		CMSG_SET_WATCHED_FACTION = 0x318,
		MSG_MOVE_TIME_SKIPPED = 0x319,
		SMSG_SPLINE_MOVE_ROOT = 0x31A,
		CMSG_SET_EXPLORATION_ALL = 0x31B,
		SMSG_INVALIDATE_PLAYER = 0x31C,
		CMSG_RESET_INSTANCES = 0x31D,
		SMSG_INSTANCE_RESET = 0x31E,
		SMSG_INSTANCE_RESET_FAILED = 0x31F,
		SMSG_UPDATE_LAST_INSTANCE = 0x320,
		MSG_RAID_TARGET_UPDATE = 0x321,
		MSG_RAID_READY_CHECK = 0x322,
		CMSG_LUA_USAGE = 0x323,
		SMSG_PET_ACTION_SOUND = 0x324,
		SMSG_PET_DISMISS_SOUND = 0x325,
		SMSG_GHOSTEE_GONE = 0x326,
		CMSG_GM_UPDATE_TICKET_STATUS = 0x327,
		SMSG_GM_TICKET_STATUS_UPDATE = 0x328,
		MSG_SET_DUNGEON_DIFFICULTY = 0x329,
		CMSG_GMSURVEY_SUBMIT = 0x32A,
		SMSG_UPDATE_INSTANCE_OWNERSHIP = 0x32B,
		CMSG_IGNORE_KNOCKBACK_CHEAT = 0x32C,
		SMSG_CHAT_PLAYER_AMBIGUOUS = 0x32D,
		MSG_DELAY_GHOST_TELEPORT = 0x32E,
		SMSG_SPELLINSTAKILLLOG = 0x32F,
		SMSG_SPELL_UPDATE_CHAIN_TARGETS = 0x330,
		CMSG_CHAT_FILTERED = 0x331,
		SMSG_EXPECTED_SPAM_RECORDS = 0x332,
		SMSG_SPELLSTEALLOG = 0x333,
		CMSG_LOTTERY_QUERY_OBSOLETE = 0x334,
		SMSG_LOTTERY_QUERY_RESULT_OBSOLETE = 0x335,
		CMSG_BUY_LOTTERY_TICKET_OBSOLETE = 0x336,
		SMSG_LOTTERY_RESULT_OBSOLETE = 0x337,
		SMSG_CHARACTER_PROFILE = 0x338,
		SMSG_CHARACTER_PROFILE_REALM_CONNECTED = 0x339,
		SMSG_DEFENSE_MESSAGE = 0x33A,
		SMSG_INSTANCE_DIFFICULTY = 0x33B,
		MSG_GM_RESETINSTANCELIMIT = 0x33C,
		SMSG_MOTD = 0x33D,
		SMSG_MOVE_SET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY = 0x33E,
		SMSG_MOVE_UNSET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY = 0x33F,
		CMSG_MOVE_SET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY_ACK = 0x340,
		MSG_MOVE_START_SWIM_CHEAT = 0x341,
		MSG_MOVE_STOP_SWIM_CHEAT = 0x342,
		SMSG_MOVE_SET_CAN_FLY = 0x343,
		SMSG_MOVE_UNSET_CAN_FLY = 0x344,
		CMSG_MOVE_SET_CAN_FLY_ACK = 0x345,
		CMSG_MOVE_SET_FLY = 0x346,
		CMSG_SOCKET_GEMS = 0x347,
		CMSG_ARENA_TEAM_CREATE = 0x348,
		SMSG_ARENA_TEAM_COMMAND_RESULT = 0x349,
		MSG_MOVE_UPDATE_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY = 0x34A,
		CMSG_ARENA_TEAM_QUERY = 0x34B,
		SMSG_ARENA_TEAM_QUERY_RESPONSE = 0x34C,
		CMSG_ARENA_TEAM_ROSTER = 0x34D,
		SMSG_ARENA_TEAM_ROSTER = 0x34E,
		CMSG_ARENA_TEAM_INVITE = 0x34F,
		SMSG_ARENA_TEAM_INVITE = 0x350,
		CMSG_ARENA_TEAM_ACCEPT = 0x351,
		CMSG_ARENA_TEAM_DECLINE = 0x352,
		CMSG_ARENA_TEAM_LEAVE = 0x353,
		CMSG_ARENA_TEAM_REMOVE = 0x354,
		CMSG_ARENA_TEAM_DISBAND = 0x355,
		CMSG_ARENA_TEAM_LEADER = 0x356,
		SMSG_ARENA_TEAM_EVENT = 0x357,
		CMSG_BATTLEMASTER_JOIN_ARENA = 0x358,
		MSG_MOVE_START_ASCEND = 0x359,
		MSG_MOVE_STOP_ASCEND = 0x35A,
		SMSG_ARENA_TEAM_STATS = 0x35B,
		CMSG_LFG_JOIN = 0x35C,
		CMSG_LFG_LEAVE = 0x35D,
		CMSG_LFG_SEARCH_JOIN = 0x35E,
		CMSG_LFG_SEARCH_LEAVE = 0x35F,
		SMSG_LFG_SEARCH_RESULTS = 0x360,
		SMSG_LFG_PROPOSAL_UPDATE = 0x361,
		CMSG_LFG_PROPOSAL_RESPONSE = 0x362,
		SMSG_LFG_ROLE_CHECK_UPDATE = 0x363,
		SMSG_LFG_JOIN_RESULT = 0x364,
		SMSG_LFG_QUEUE_STATUS = 0x365,
		CMSG_SET_LFG_COMMENT = 0x366,
		SMSG_LFG_UPDATE_PLAYER = 0x367,
		SMSG_LFG_UPDATE_PARTY = 0x368,
		SMSG_LFG_UPDATE_SEARCH = 0x369,
		CMSG_LFG_SET_ROLES = 0x36A,
		CMSG_LFG_SET_NEEDS = 0x36B,
		CMSG_LFG_BOOT_PLAYER_VOTE = 0x36C,
		SMSG_LFG_BOOT_PLAYER = 0x36D,
		CMSG_LFG_GET_PLAYER_INFO = 0x36E,
		SMSG_LFG_PLAYER_INFO = 0x36F,
		CMSG_LFG_TELEPORT = 0x370,
		CMSG_LFG_GET_PARTY_INFO = 0x371,
		SMSG_LFG_PARTY_INFO = 0x372,
		SMSG_TITLE_EARNED = 0x373,
		CMSG_SET_TITLE = 0x374,
		CMSG_CANCEL_MOUNT_AURA = 0x375,
		SMSG_ARENA_ERROR = 0x376,
		MSG_INSPECT_ARENA_TEAMS = 0x377,
		SMSG_DEATH_RELEASE_LOC = 0x378,
		CMSG_CANCEL_TEMP_ENCHANTMENT = 0x379,
		SMSG_FORCED_DEATH_UPDATE = 0x37A,
		CMSG_CHEAT_SET_HONOR_CURRENCY = 0x37B,
		CMSG_CHEAT_SET_ARENA_CURRENCY = 0x37C,
		MSG_MOVE_SET_FLIGHT_SPEED_CHEAT = 0x37D,
		MSG_MOVE_SET_FLIGHT_SPEED = 0x37E,
		MSG_MOVE_SET_FLIGHT_BACK_SPEED_CHEAT = 0x37F,
		MSG_MOVE_SET_FLIGHT_BACK_SPEED = 0x380,
		SMSG_FORCE_FLIGHT_SPEED_CHANGE = 0x381,
		CMSG_FORCE_FLIGHT_SPEED_CHANGE_ACK = 0x382,
		SMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE = 0x383,
		CMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE_ACK = 0x384,
		SMSG_SPLINE_SET_FLIGHT_SPEED = 0x385,
		SMSG_SPLINE_SET_FLIGHT_BACK_SPEED = 0x386,
		CMSG_MAELSTROM_INVALIDATE_CACHE = 0x387,
		SMSG_FLIGHT_SPLINE_SYNC = 0x388,
		CMSG_SET_TAXI_BENCHMARK_MODE = 0x389,
		SMSG_JOINED_BATTLEGROUND_QUEUE = 0x38A,
		SMSG_REALM_SPLIT = 0x38B,
		CMSG_REALM_SPLIT = 0x38C,
		CMSG_MOVE_CHNG_TRANSPORT = 0x38D,
		MSG_PARTY_ASSIGNMENT = 0x38E,
		SMSG_OFFER_PETITION_ERROR = 0x38F,
		SMSG_TIME_SYNC_REQ = 0x390,
		CMSG_TIME_SYNC_RESP = 0x391,
		CMSG_SEND_LOCAL_EVENT = 0x392,
		CMSG_SEND_GENERAL_TRIGGER = 0x393,
		CMSG_SEND_COMBAT_TRIGGER = 0x394,
		CMSG_MAELSTROM_GM_SENT_MAIL = 0x395,
		SMSG_RESET_FAILED_NOTIFY = 0x396,
		SMSG_REAL_GROUP_UPDATE = 0x397,
		SMSG_LFG_DISABLED = 0x398,
		CMSG_ACTIVE_PVP_CHEAT = 0x399,
		CMSG_CHEAT_DUMP_ITEMS_DEBUG_ONLY = 0x39A,
		SMSG_CHEAT_DUMP_ITEMS_DEBUG_ONLY_RESPONSE = 0x39B,
		SMSG_CHEAT_DUMP_ITEMS_DEBUG_ONLY_RESPONSE_WRITE_FILE = 0x39C,
		SMSG_UPDATE_COMBO_POINTS = 0x39D,
		SMSG_VOICE_SESSION_ROSTER_UPDATE = 0x39E,
		SMSG_VOICE_SESSION_LEAVE = 0x39F,
		SMSG_VOICE_SESSION_ADJUST_PRIORITY = 0x3A0,
		CMSG_VOICE_SET_TALKER_MUTED_REQUEST = 0x3A1,
		SMSG_VOICE_SET_TALKER_MUTED = 0x3A2,
		SMSG_INIT_EXTRA_AURA_INFO_OBSOLETE = 0x3A3,
		SMSG_SET_EXTRA_AURA_INFO_OBSOLETE = 0x3A4,
		SMSG_SET_EXTRA_AURA_INFO_NEED_UPDATE_OBSOLETE = 0x3A5,
		SMSG_CLEAR_EXTRA_AURA_INFO_OBSOLETE = 0x3A6,
		MSG_MOVE_START_DESCEND = 0x3A7,
		CMSG_IGNORE_REQUIREMENTS_CHEAT = 0x3A8,
		SMSG_IGNORE_REQUIREMENTS_CHEAT = 0x3A9,
		SMSG_SPELL_CHANCE_PROC_LOG = 0x3AA,
		CMSG_MOVE_SET_RUN_SPEED = 0x3AB,
		SMSG_DISMOUNT = 0x3AC,
		MSG_MOVE_UPDATE_CAN_FLY = 0x3AD,
		MSG_RAID_READY_CHECK_CONFIRM = 0x3AE,
		CMSG_VOICE_SESSION_ENABLE = 0x3AF,
		SMSG_VOICE_SESSION_ENABLE = 0x3B0,
		SMSG_VOICE_PARENTAL_CONTROLS = 0x3B1,
		CMSG_GM_WHISPER = 0x3B2,
		SMSG_GM_MESSAGECHAT = 0x3B3,
		MSG_GM_GEARRATING = 0x3B4,
		CMSG_COMMENTATOR_ENABLE = 0x3B5,
		SMSG_COMMENTATOR_STATE_CHANGED = 0x3B6,
		CMSG_COMMENTATOR_GET_MAP_INFO = 0x3B7,
		SMSG_COMMENTATOR_MAP_INFO = 0x3B8,
		CMSG_COMMENTATOR_GET_PLAYER_INFO = 0x3B9,
		SMSG_COMMENTATOR_GET_PLAYER_INFO = 0x3BA,
		SMSG_COMMENTATOR_PLAYER_INFO = 0x3BB,
		CMSG_COMMENTATOR_ENTER_INSTANCE = 0x3BC,
		CMSG_COMMENTATOR_EXIT_INSTANCE = 0x3BD,
		CMSG_COMMENTATOR_INSTANCE_COMMAND = 0x3BE,
		SMSG_CLEAR_TARGET = 0x3BF,
		CMSG_BOT_DETECTED = 0x3C0,
		SMSG_CROSSED_INEBRIATION_THRESHOLD = 0x3C1,
		CMSG_CHEAT_PLAYER_LOGIN = 0x3C2,
		CMSG_CHEAT_PLAYER_LOOKUP = 0x3C3,
		SMSG_CHEAT_PLAYER_LOOKUP = 0x3C4,
		SMSG_KICK_REASON = 0x3C5,
		MSG_RAID_READY_CHECK_FINISHED = 0x3C6,
		CMSG_COMPLAIN = 0x3C7,
		SMSG_COMPLAIN_RESULT = 0x3C8,
		SMSG_FEATURE_SYSTEM_STATUS = 0x3C9,
		CMSG_GM_SHOW_COMPLAINTS = 0x3CA,
		CMSG_GM_UNSQUELCH = 0x3CB,
		CMSG_CHANNEL_SILENCE_VOICE = 0x3CC,
		CMSG_CHANNEL_SILENCE_ALL = 0x3CD,
		CMSG_CHANNEL_UNSILENCE_VOICE = 0x3CE,
		CMSG_CHANNEL_UNSILENCE_ALL = 0x3CF,
		CMSG_TARGET_CAST = 0x3D0,
		CMSG_TARGET_SCRIPT_CAST = 0x3D1,
		CMSG_CHANNEL_DISPLAY_LIST = 0x3D2,
		CMSG_SET_ACTIVE_VOICE_CHANNEL = 0x3D3,
		CMSG_GET_CHANNEL_MEMBER_COUNT = 0x3D4,
		SMSG_CHANNEL_MEMBER_COUNT = 0x3D5,
		CMSG_CHANNEL_VOICE_ON = 0x3D6,
		CMSG_CHANNEL_VOICE_OFF = 0x3D7,
		CMSG_DEBUG_LIST_TARGETS = 0x3D8,
		SMSG_DEBUG_LIST_TARGETS = 0x3D9,
		SMSG_AVAILABLE_VOICE_CHANNEL = 0x3DA,
		CMSG_ADD_VOICE_IGNORE = 0x3DB,
		CMSG_DEL_VOICE_IGNORE = 0x3DC,
		CMSG_PARTY_SILENCE = 0x3DD,
		CMSG_PARTY_UNSILENCE = 0x3DE,
		MSG_NOTIFY_PARTY_SQUELCH = 0x3DF,
		SMSG_COMSAT_RECONNECT_TRY = 0x3E0,
		SMSG_COMSAT_DISCONNECT = 0x3E1,
		SMSG_COMSAT_CONNECT_FAIL = 0x3E2,
		SMSG_VOICE_CHAT_STATUS = 0x3E3,
		CMSG_REPORT_PVP_AFK = 0x3E4,
		SMSG_REPORT_PVP_AFK_RESULT = 0x3E5,
		CMSG_GUILD_BANKER_ACTIVATE = 0x3E6,
		CMSG_GUILD_BANK_QUERY_TAB = 0x3E7,
		SMSG_GUILD_BANK_LIST = 0x3E8,
		CMSG_GUILD_BANK_SWAP_ITEMS = 0x3E9,
		CMSG_GUILD_BANK_BUY_TAB = 0x3EA,
		CMSG_GUILD_BANK_UPDATE_TAB = 0x3EB,
		CMSG_GUILD_BANK_DEPOSIT_MONEY = 0x3EC,
		CMSG_GUILD_BANK_WITHDRAW_MONEY = 0x3ED,
		MSG_GUILD_BANK_LOG_QUERY = 0x3EE,
		CMSG_SET_CHANNEL_WATCH = 0x3EF,
		SMSG_USERLIST_ADD = 0x3F0,
		SMSG_USERLIST_REMOVE = 0x3F1,
		SMSG_USERLIST_UPDATE = 0x3F2,
		CMSG_CLEAR_CHANNEL_WATCH = 0x3F3,
		SMSG_INSPECT_RESULTS = 0x3F4,
		SMSG_GOGOGO_OBSOLETE = 0x3F5,
		SMSG_ECHO_PARTY_SQUELCH = 0x3F6,
		CMSG_SET_TITLE_SUFFIX = 0x3F7,
		CMSG_SPELLCLICK = 0x3F8,
		SMSG_LOOT_LIST = 0x3F9,
		CMSG_GM_CHARACTER_RESTORE = 0x3FA,
		CMSG_GM_CHARACTER_SAVE = 0x3FB,
		SMSG_VOICESESSION_FULL = 0x3FC,
		MSG_GUILD_PERMISSIONS = 0x3FD,
		MSG_GUILD_BANK_MONEY_WITHDRAWN = 0x3FE,
		MSG_GUILD_EVENT_LOG_QUERY = 0x3FF,
		CMSG_MAELSTROM_RENAME_GUILD = 0x400,
		CMSG_GET_MIRRORIMAGE_DATA = 0x401,
		SMSG_MIRRORIMAGE_DATA = 0x402,
		SMSG_FORCE_DISPLAY_UPDATE = 0x403,
		SMSG_SPELL_CHANCE_RESIST_PUSHBACK = 0x404,
		CMSG_IGNORE_DIMINISHING_RETURNS_CHEAT = 0x405,
		SMSG_IGNORE_DIMINISHING_RETURNS_CHEAT = 0x406,
		CMSG_KEEP_ALIVE = 0x407,
		SMSG_RAID_READY_CHECK_ERROR = 0x408,
		CMSG_OPT_OUT_OF_LOOT = 0x409,
		MSG_QUERY_GUILD_BANK_TEXT = 0x40A,
		CMSG_SET_GUILD_BANK_TEXT = 0x40B,
		CMSG_SET_GRANTABLE_LEVELS = 0x40C,
		CMSG_GRANT_LEVEL = 0x40D,
		CMSG_REFER_A_FRIEND = 0x40E,
		MSG_GM_CHANGE_ARENA_RATING = 0x40F,
		CMSG_DECLINE_CHANNEL_INVITE = 0x410,
		SMSG_GROUPACTION_THROTTLED = 0x411,
		SMSG_OVERRIDE_LIGHT = 0x412,
		SMSG_TOTEM_CREATED = 0x413,
		CMSG_TOTEM_DESTROYED = 0x414,
		CMSG_EXPIRE_RAID_INSTANCE = 0x415,
		CMSG_NO_SPELL_VARIANCE = 0x416,
		CMSG_QUESTGIVER_STATUS_MULTIPLE_QUERY = 0x417,
		SMSG_QUESTGIVER_STATUS_MULTIPLE = 0x418,
		CMSG_SET_PLAYER_DECLINED_NAMES = 0x419,
		SMSG_SET_PLAYER_DECLINED_NAMES_RESULT = 0x41A,
		CMSG_QUERY_SERVER_BUCK_DATA = 0x41B,
		CMSG_CLEAR_SERVER_BUCK_DATA = 0x41C,
		SMSG_SERVER_BUCK_DATA = 0x41D,
		SMSG_SEND_UNLEARN_SPELLS = 0x41E,
		SMSG_PROPOSE_LEVEL_GRANT = 0x41F,
		CMSG_ACCEPT_LEVEL_GRANT = 0x420,
		SMSG_REFER_A_FRIEND_FAILURE = 0x421,
		SMSG_SPLINE_MOVE_SET_FLYING = 0x422,
		SMSG_SPLINE_MOVE_UNSET_FLYING = 0x423,
		SMSG_SUMMON_CANCEL = 0x424,
		CMSG_CHANGE_PERSONAL_ARENA_RATING = 0x425,
		CMSG_ALTER_APPEARANCE = 0x426,
		SMSG_ENABLE_BARBER_SHOP = 0x427,
		SMSG_BARBER_SHOP_RESULT = 0x428,
		CMSG_CALENDAR_GET_CALENDAR = 0x429,
		CMSG_CALENDAR_GET_EVENT = 0x42A,
		CMSG_CALENDAR_GUILD_FILTER = 0x42B,
		CMSG_CALENDAR_ARENA_TEAM = 0x42C,
		CMSG_CALENDAR_ADD_EVENT = 0x42D,
		CMSG_CALENDAR_UPDATE_EVENT = 0x42E,
		CMSG_CALENDAR_REMOVE_EVENT = 0x42F,
		CMSG_CALENDAR_COPY_EVENT = 0x430,
		CMSG_CALENDAR_EVENT_INVITE = 0x431,
		CMSG_CALENDAR_EVENT_RSVP = 0x432,
		CMSG_CALENDAR_EVENT_REMOVE_INVITE = 0x433,
		CMSG_CALENDAR_EVENT_STATUS = 0x434,
		CMSG_CALENDAR_EVENT_MODERATOR_STATUS = 0x435,
		SMSG_CALENDAR_SEND_CALENDAR = 0x436,
		SMSG_CALENDAR_SEND_EVENT = 0x437,
		SMSG_CALENDAR_FILTER_GUILD = 0x438,
		SMSG_CALENDAR_ARENA_TEAM = 0x439,
		SMSG_CALENDAR_EVENT_INVITE = 0x43A,
		SMSG_CALENDAR_EVENT_INVITE_REMOVED = 0x43B,
		SMSG_CALENDAR_EVENT_STATUS = 0x43C,
		SMSG_CALENDAR_COMMAND_RESULT = 0x43D,
		SMSG_CALENDAR_RAID_LOCKOUT_ADDED = 0x43E,
		SMSG_CALENDAR_RAID_LOCKOUT_REMOVED = 0x43F,
		SMSG_CALENDAR_EVENT_INVITE_ALERT = 0x440,
		SMSG_CALENDAR_EVENT_INVITE_REMOVED_ALERT = 0x441,
		SMSG_CALENDAR_EVENT_INVITE_STATUS_ALERT = 0x442,
		SMSG_CALENDAR_EVENT_REMOVED_ALERT = 0x443,
		SMSG_CALENDAR_EVENT_UPDATED_ALERT = 0x444,
		SMSG_CALENDAR_EVENT_MODERATOR_STATUS_ALERT = 0x445,
		CMSG_CALENDAR_COMPLAIN = 0x446,
		CMSG_CALENDAR_GET_NUM_PENDING = 0x447,
		SMSG_CALENDAR_SEND_NUM_PENDING = 0x448,
		CMSG_SAVE_DANCE = 0x449,
		SMSG_NOTIFY_DANCE = 0x44A,
		CMSG_PLAY_DANCE = 0x44B,
		SMSG_PLAY_DANCE = 0x44C,
		CMSG_LOAD_DANCES = 0x44D,
		CMSG_STOP_DANCE = 0x44E,
		SMSG_STOP_DANCE = 0x44F,
		CMSG_SYNC_DANCE = 0x450,
		CMSG_DANCE_QUERY = 0x451,
		SMSG_DANCE_QUERY_RESPONSE = 0x452,
		SMSG_INVALIDATE_DANCE = 0x453,
		CMSG_DELETE_DANCE = 0x454,
		SMSG_LEARNED_DANCE_MOVES = 0x455,
		CMSG_LEARN_DANCE_MOVE = 0x456,
		CMSG_UNLEARN_DANCE_MOVE = 0x457,
		CMSG_SET_RUNE_COUNT = 0x458,
		CMSG_SET_RUNE_COOLDOWN = 0x459,
		MSG_MOVE_SET_PITCH_RATE_CHEAT = 0x45A,
		MSG_MOVE_SET_PITCH_RATE = 0x45B,
		SMSG_FORCE_PITCH_RATE_CHANGE = 0x45C,
		CMSG_FORCE_PITCH_RATE_CHANGE_ACK = 0x45D,
		SMSG_SPLINE_SET_PITCH_RATE = 0x45E,
		CMSG_CALENDAR_EVENT_INVITE_NOTES = 0x45F,
		SMSG_CALENDAR_EVENT_INVITE_NOTES = 0x460,
		SMSG_CALENDAR_EVENT_INVITE_NOTES_ALERT = 0x461,
		CMSG_UPDATE_MISSILE_TRAJECTORY = 0x462,
		SMSG_UPDATE_ACCOUNT_DATA_COMPLETE = 0x463,
		SMSG_TRIGGER_MOVIE = 0x464,
		CMSG_COMPLETE_MOVIE = 0x465,
		CMSG_SET_GLYPH_SLOT = 0x466,
		CMSG_SET_GLYPH = 0x467,
		SMSG_ACHIEVEMENT_EARNED = 0x468,
		SMSG_DYNAMIC_DROP_ROLL_RESULT = 0x469,
		SMSG_CRITERIA_UPDATE = 0x46A,
		CMSG_QUERY_INSPECT_ACHIEVEMENTS = 0x46B,
		SMSG_RESPOND_INSPECT_ACHIEVEMENTS = 0x46C,
		CMSG_DISMISS_CONTROLLED_VEHICLE = 0x46D,
		CMSG_COMPLETE_ACHIEVEMENT_CHEAT = 0x46E,
		SMSG_QUESTUPDATE_ADD_PVP_KILL = 0x46F,
		CMSG_SET_CRITERIA_CHEAT = 0x470,
		SMSG_CALENDAR_RAID_LOCKOUT_UPDATED = 0x471,
		CMSG_UNITANIMTIER_CHEAT = 0x472,
		CMSG_CHAR_CUSTOMIZE = 0x473,
		SMSG_CHAR_CUSTOMIZE = 0x474,
		SMSG_PET_RENAMEABLE = 0x475,
		CMSG_REQUEST_VEHICLE_EXIT = 0x476,
		CMSG_REQUEST_VEHICLE_PREV_SEAT = 0x477,
		CMSG_REQUEST_VEHICLE_NEXT_SEAT = 0x478,
		CMSG_REQUEST_VEHICLE_SWITCH_SEAT = 0x479,
		CMSG_PET_LEARN_TALENT = 0x47A,
		CMSG_PET_UNLEARN_TALENTS = 0x47B,
		SMSG_SET_PHASE_SHIFT = 0x47C,
		SMSG_ALL_ACHIEVEMENT_DATA = 0x47D,
		CMSG_FORCE_SAY_CHEAT = 0x47E,
		SMSG_HEALTH_UPDATE = 0x47F,
		SMSG_POWER_UPDATE = 0x480,
		CMSG_GAMEOBJ_REPORT_USE = 0x481,
		SMSG_HIGHEST_THREAT_UPDATE = 0x482,
		SMSG_THREAT_UPDATE = 0x483,
		SMSG_THREAT_REMOVE = 0x484,
		SMSG_THREAT_CLEAR = 0x485,
		SMSG_CONVERT_RUNE = 0x486,
		SMSG_RESYNC_RUNES = 0x487,
		SMSG_ADD_RUNE_POWER = 0x488,
		CMSG_START_QUEST = 0x489,
		CMSG_REMOVE_GLYPH = 0x48A,
		CMSG_DUMP_OBJECTS = 0x48B,
		SMSG_DUMP_OBJECTS_DATA = 0x48C,
		CMSG_DISMISS_CRITTER = 0x48D,
		SMSG_NOTIFY_DEST_LOC_SPELL_CAST = 0x48E,
		CMSG_AUCTION_LIST_PENDING_SALES = 0x48F,
		SMSG_AUCTION_LIST_PENDING_SALES = 0x490,
		SMSG_MODIFY_COOLDOWN = 0x491,
		SMSG_PET_UPDATE_COMBO_POINTS = 0x492,
		CMSG_ENABLETAXI = 0x493,
		SMSG_PRE_RESURRECT = 0x494,
		SMSG_AURA_UPDATE_ALL = 0x495,
		SMSG_AURA_UPDATE = 0x496,
		CMSG_FLOOD_GRACE_CHEAT = 0x497,
		SMSG_SERVER_FIRST_ACHIEVEMENT = 0x498,
		SMSG_PET_LEARNED_SPELL = 0x499,
		SMSG_PET_REMOVED_SPELL = 0x49A,
		CMSG_CHANGE_SEATS_ON_CONTROLLED_VEHICLE = 0x49B,
		CMSG_HEARTH_AND_RESURRECT = 0x49C,
		SMSG_ON_CANCEL_EXPECTED_RIDE_VEHICLE_AURA = 0x49D,
		SMSG_CRITERIA_DELETED = 0x49E,
		SMSG_ACHIEVEMENT_DELETED = 0x49F,
		CMSG_SERVER_INFO_QUERY = 0x4A0,
		SMSG_SERVER_INFO_RESPONSE = 0x4A1,
		CMSG_CHECK_LOGIN_CRITERIA = 0x4A2,
		SMSG_SERVER_BUCK_DATA_START = 0x4A3,
		CMSG_SET_BREATH = 0x4A4,
		CMSG_QUERY_VEHICLE_STATUS = 0x4A5,
		SMSG_BATTLEGROUND_INFO_THROTTLED = 0x4A6,
		SMSG_SET_VEHICLE_REC_ID = 0x4A7,
		CMSG_RIDE_VEHICLE_INTERACT = 0x4A8,
		CMSG_CONTROLLER_EJECT_PASSENGER = 0x4A9,
		SMSG_PET_GUIDS = 0x4AA,
		SMSG_CLIENTCACHE_VERSION = 0x4AB,
		CMSG_CHANGE_GDF_ARENA_RATING = 0x4AC,
		CMSG_SET_ARENA_TEAM_RATING_BY_INDEX = 0x4AD,
		CMSG_SET_ARENA_TEAM_WEEKLY_GAMES = 0x4AE,
		CMSG_SET_ARENA_TEAM_SEASON_GAMES = 0x4AF,
		CMSG_SET_ARENA_MEMBER_WEEKLY_GAMES = 0x4B0,
		CMSG_SET_ARENA_MEMBER_SEASON_GAMES = 0x4B1,
		SMSG_SET_ITEM_PURCHASE_DATA = 0x4B2,
		CMSG_GET_ITEM_PURCHASE_DATA = 0x4B3,
		CMSG_ITEM_PURCHASE_REFUND = 0x4B4,
		SMSG_ITEM_PURCHASE_REFUND_RESULT = 0x4B5,
		CMSG_CORPSE_TRANSPORT_QUERY = 0x4B6,
		SMSG_CORPSE_TRANSPORT_QUERY = 0x4B7,
		CMSG_UNUSED5 = 0x4B8,
		CMSG_UNUSED6 = 0x4B9,
		CMSG_CALENDAR_EVENT_SIGNUP = 0x4BA,
		SMSG_CALENDAR_CLEAR_PENDING_ACTION = 0x4BB,
		SMSG_LOAD_EQUIPMENT_SET = 0x4BC,
		CMSG_SAVE_EQUIPMENT_SET = 0x4BD,
		CMSG_ON_MISSILE_TRAJECTORY_COLLISION = 0x4BE,
		SMSG_NOTIFY_MISSILE_TRAJECTORY_COLLISION = 0x4BF,
		SMSG_TALENT_UPDATE = 0x4C0,
		CMSG_LEARN_TALENT_GROUP = 0x4C1,
		CMSG_PET_LEARN_TALENT_GROUP = 0x4C2,
		CMSG_SET_ACTIVE_TALENT_GROUP_OBSOLETE = 0x4C3,
		CMSG_GM_GRANT_ACHIEVEMENT = 0x4C4,
		CMSG_GM_REMOVE_ACHIEVEMENT = 0x4C5,
		CMSG_GM_SET_CRITERIA_FOR_PLAYER = 0x4C6,
		SMSG_DESTROY_ARENA_UNIT = 0x4C7,
		SMSG_ARENA_TEAM_CHANGE_FAILED = 0x4C8,
		CMSG_PROFILEDATA_REQUEST = 0x4C9,
		SMSG_PROFILEDATA_RESPONSE = 0x4CA,
		CMSG_START_BATTLEFIELD_CHEAT = 0x4CB,
		CMSG_END_BATTLEFIELD_CHEAT = 0x4CC,
		SMSG_COMPOUND_MOVE = 0x4CD,
		SMSG_MOVE_GRAVITY_DISABLE = 0x4CE,
		CMSG_MOVE_GRAVITY_DISABLE_ACK = 0x4CF,
		SMSG_MOVE_GRAVITY_ENABLE = 0x4D0,
		CMSG_MOVE_GRAVITY_ENABLE_ACK = 0x4D1,
		MSG_MOVE_GRAVITY_CHNG = 0x4D2,
		SMSG_SPLINE_MOVE_GRAVITY_DISABLE = 0x4D3,
		SMSG_SPLINE_MOVE_GRAVITY_ENABLE = 0x4D4,
		CMSG_USE_EQUIPMENT_SET = 0x4D5,
		SMSG_USE_EQUIPMENT_SET_RESULT = 0x4D6,
		CMSG_FORCE_ANIM = 0x4D7,
		SMSG_FORCE_ANIM = 0x4D8,
		CMSG_CHAR_FACTION_CHANGE = 0x4D9,
		SMSG_CHAR_FACTION_CHANGE = 0x4DA,
		CMSG_PVP_QUEUE_STATS_REQUEST = 0x4DB,
		SMSG_PVP_QUEUE_STATS = 0x4DC,
		CMSG_SET_PAID_SERVICE_CHEAT = 0x4DD,
		SMSG_BATTLEFIELD_MANAGER_ENTRY_INVITE = 0x4DE,
		CMSG_BATTLEFIELD_MANAGER_ENTRY_INVITE_RESPONSE = 0x4DF,
		SMSG_BATTLEFIELD_MANAGER_ENTERING = 0x4E0,
		SMSG_BATTLEFIELD_MANAGER_QUEUE_INVITE = 0x4E1,
		CMSG_BATTLEFIELD_MANAGER_QUEUE_INVITE_RESPONSE = 0x4E2,
		CMSG_BATTLEFIELD_MANAGER_QUEUE_REQUEST = 0x4E3,
		SMSG_BATTLEFIELD_MANAGER_QUEUE_REQUEST_RESPONSE = 0x4E4,
		SMSG_BATTLEFIELD_MANAGER_EJECT_PENDING = 0x4E5,
		SMSG_BATTLEFIELD_MANAGER_EJECTED = 0x4E6,
		CMSG_BATTLEFIELD_MANAGER_EXIT_REQUEST = 0x4E7,
		SMSG_BATTLEFIELD_MANAGER_STATE_CHANGED = 0x4E8,
		CMSG_BATTLEFIELD_MANAGER_ADVANCE_STATE = 0x4E9,
		CMSG_BATTLEFIELD_MANAGER_SET_NEXT_TRANSITION_TIME = 0x4EA,
		MSG_SET_RAID_DIFFICULTY = 0x4EB,
		CMSG_XPGAIN = 0x4EC,
		SMSG_XPGAIN = 0x4ED,
		SMSG_GMTICKET_RESPONSE_ERROR = 0x4EE,
		SMSG_GMTICKET_GET_RESPONSE = 0x4EF,
		CMSG_GMTICKET_RESOLVE_RESPONSE = 0x4F0,
		SMSG_GMTICKET_RESOLVE_RESPONSE = 0x4F1,
		SMSG_GMTICKET_CREATE_RESPONSE_TICKET = 0x4F2,
		CMSG_GM_CREATE_TICKET_RESPONSE = 0x4F3,
		CMSG_SERVERINFO = 0x4F4,
		SMSG_SERVERINFO = 0x4F5,
		CMSG_UI_TIME_REQUEST = 0x4F6,
		SMSG_UI_TIME = 0x4F7,
		CMSG_CHAR_RACE_CHANGE = 0x4F8,
		MSG_VIEW_PHASE_SHIFT = 0x4F9,
		SMSG_TALENTS_INVOLUNTARILY_RESET = 0x4FA,
		CMSG_DEBUG_SERVER_GEO = 0x4FB,
		SMSG_DEBUG_SERVER_GEO = 0x4FC,
		SMSG_LOOT_UPDATE = 0x4FD,
		UMSG_UPDATE_GROUP_INFO = 0x4FE,
		CMSG_READY_FOR_ACCOUNT_DATA_TIMES = 0x4FF,
		CMSG_QUERY_GET_ALL_QUESTS = 0x500,
		SMSG_ALL_QUESTS_COMPLETED = 0x501,
		CMSG_GMLAGREPORT_SUBMIT = 0x502,
		CMSG_AFK_MONITOR_INFO_REQUEST = 0x503,
		SMSG_AFK_MONITOR_INFO_RESPONSE = 0x504,
		CMSG_AFK_MONITOR_INFO_CLEAR = 0x505,
		SMSG_AREA_TRIGGER_NO_CORPSE = 0x506,
		CMSG_GM_NUKE_CHARACTER = 0x507,
		CMSG_LOW_LEVEL_RAID = 0x508,
		CMSG_LOW_LEVEL_RAID_USER = 0x509,
		SMSG_CAMERA_SHAKE = 0x50A,
		SMSG_SOCKET_GEMS = 0x50B,
		CMSG_SET_CHARACTER_MODEL = 0x50C,
		SMSG_CONNECT_TO = 0x50D,
		CMSG_CONNECT_TO_FAILED = 0x50E,
		SMSG_SUSPEND_COMMS = 0x50F,
		CMSG_SUSPEND_COMMS_ACK = 0x510,
		SMSG_RESUME_COMMS = 0x511,
		CMSG_AUTH_CONTINUED_SESSION = 0x512,
		CMSG_DROP_NEW_CONNECTION = 0x513,
		SMSG_SEND_ALL_COMBAT_LOG = 0x514,
		SMSG_OPEN_LFG_DUNGEON_FINDER = 0x515,
		SMSG_MOVE_SET_COLLISION_HGT = 0x516,
		CMSG_MOVE_SET_COLLISION_HGT_ACK = 0x517,
		MSG_MOVE_SET_COLLISION_HGT = 0x518,
		CMSG_CLEAR_RANDOM_BG_WIN_TIME = 0x519,
		CMSG_CLEAR_HOLIDAY_BG_WIN_TIME = 0x51A,
		CMSG_COMMENTATOR_SKIRMISH_QUEUE_COMMAND = 0x51B,
		SMSG_COMMENTATOR_SKIRMISH_QUEUE_RESULT1 = 0x51C,
		SMSG_COMMENTATOR_SKIRMISH_QUEUE_RESULT2 = 0x51D,
		SMSG_COMPRESSED_UNKNOWN_1310 = 0x51E,
		NUM_MSG_TYPES = 0x51F,
	};

	using namespace std::literals::string_view_literals;
	constexpr auto opcode_names = std::array{
		"MSG_NULL_ACTION"sv,
		"CMSG_BOOTME"sv,
		"CMSG_DBLOOKUP"sv,
		"SMSG_DBLOOKUP"sv,
		"CMSG_QUERY_OBJECT_POSITION"sv,
		"SMSG_QUERY_OBJECT_POSITION"sv,
		"CMSG_QUERY_OBJECT_ROTATION"sv,
		"SMSG_QUERY_OBJECT_ROTATION"sv,
		"CMSG_WORLD_TELEPORT"sv,
		"CMSG_TELEPORT_TO_UNIT"sv,
		"CMSG_ZONE_MAP"sv,
		"SMSG_ZONE_MAP"sv,
		"CMSG_DEBUG_CHANGECELLZONE"sv,
		"CMSG_MOVE_CHARACTER_CHEAT"sv,
		"SMSG_MOVE_CHARACTER_CHEAT"sv,
		"CMSG_RECHARGE"sv,
		"CMSG_LEARN_SPELL"sv,
		"CMSG_CREATEMONSTER"sv,
		"CMSG_DESTROYMONSTER"sv,
		"CMSG_CREATEITEM"sv,
		"CMSG_CREATEGAMEOBJECT"sv,
		"SMSG_CHECK_FOR_BOTS"sv,
		"CMSG_MAKEMONSTERATTACKGUID"sv,
		"CMSG_BOT_DETECTED2"sv,
		"CMSG_FORCEACTION"sv,
		"CMSG_FORCEACTIONONOTHER"sv,
		"CMSG_FORCEACTIONSHOW"sv,
		"SMSG_FORCEACTIONSHOW"sv,
		"CMSG_PETGODMODE"sv,
		"SMSG_PETGODMODE"sv,
		"SMSG_REFER_A_FRIEND_EXPIRED"sv,
		"CMSG_WEATHER_SPEED_CHEAT"sv,
		"CMSG_UNDRESSPLAYER"sv,
		"CMSG_BEASTMASTER"sv,
		"CMSG_GODMODE"sv,
		"SMSG_GODMODE"sv,
		"CMSG_CHEAT_SETMONEY"sv,
		"CMSG_LEVEL_CHEAT"sv,
		"CMSG_PET_LEVEL_CHEAT"sv,
		"CMSG_SET_WORLDSTATE"sv,
		"CMSG_COOLDOWN_CHEAT"sv,
		"CMSG_USE_SKILL_CHEAT"sv,
		"CMSG_FLAG_QUEST"sv,
		"CMSG_FLAG_QUEST_FINISH"sv,
		"CMSG_CLEAR_QUEST"sv,
		"CMSG_SEND_EVENT"sv,
		"CMSG_DEBUG_AISTATE"sv,
		"SMSG_DEBUG_AISTATE"sv,
		"CMSG_DISABLE_PVP_CHEAT"sv,
		"CMSG_ADVANCE_SPAWN_TIME"sv,
		"SMSG_DESTRUCTIBLE_BUILDING_DAMAGE"sv,
		"CMSG_AUTH_SRP6_BEGIN"sv,
		"CMSG_AUTH_SRP6_PROOF"sv,
		"CMSG_AUTH_SRP6_RECODE"sv,
		"CMSG_CHAR_CREATE"sv,
		"CMSG_CHAR_ENUM"sv,
		"CMSG_CHAR_DELETE"sv,
		"SMSG_AUTH_SRP6_RESPONSE"sv,
		"SMSG_CHAR_CREATE"sv,
		"SMSG_CHAR_ENUM"sv,
		"SMSG_CHAR_DELETE"sv,
		"CMSG_PLAYER_LOGIN"sv,
		"SMSG_NEW_WORLD"sv,
		"SMSG_TRANSFER_PENDING"sv,
		"SMSG_TRANSFER_ABORTED"sv,
		"SMSG_CHARACTER_LOGIN_FAILED"sv,
		"SMSG_LOGIN_SETTIMESPEED"sv,
		"SMSG_GAMETIME_UPDATE"sv,
		"CMSG_GAMETIME_SET"sv,
		"SMSG_GAMETIME_SET"sv,
		"CMSG_GAMESPEED_SET"sv,
		"SMSG_GAMESPEED_SET"sv,
		"CMSG_SERVERTIME"sv,
		"SMSG_SERVERTIME"sv,
		"CMSG_PLAYER_LOGOUT"sv,
		"CMSG_LOGOUT_REQUEST"sv,
		"SMSG_LOGOUT_RESPONSE"sv,
		"SMSG_LOGOUT_COMPLETE"sv,
		"CMSG_LOGOUT_CANCEL"sv,
		"SMSG_LOGOUT_CANCEL_ACK"sv,
		"CMSG_NAME_QUERY"sv,
		"SMSG_NAME_QUERY_RESPONSE"sv,
		"CMSG_PET_NAME_QUERY"sv,
		"SMSG_PET_NAME_QUERY_RESPONSE"sv,
		"CMSG_GUILD_QUERY"sv,
		"SMSG_GUILD_QUERY_RESPONSE"sv,
		"CMSG_ITEM_QUERY_SINGLE"sv,
		"CMSG_ITEM_QUERY_MULTIPLE"sv,
		"SMSG_ITEM_QUERY_SINGLE_RESPONSE"sv,
		"SMSG_ITEM_QUERY_MULTIPLE_RESPONSE"sv,
		"CMSG_PAGE_TEXT_QUERY"sv,
		"SMSG_PAGE_TEXT_QUERY_RESPONSE"sv,
		"CMSG_QUEST_QUERY"sv,
		"SMSG_QUEST_QUERY_RESPONSE"sv,
		"CMSG_GAMEOBJECT_QUERY"sv,
		"SMSG_GAMEOBJECT_QUERY_RESPONSE"sv,
		"CMSG_CREATURE_QUERY"sv,
		"SMSG_CREATURE_QUERY_RESPONSE"sv,
		"CMSG_WHO"sv,
		"SMSG_WHO"sv,
		"CMSG_WHOIS"sv,
		"SMSG_WHOIS"sv,
		"CMSG_CONTACT_LIST"sv,
		"SMSG_CONTACT_LIST"sv,
		"SMSG_FRIEND_STATUS"sv,
		"CMSG_ADD_FRIEND"sv,
		"CMSG_DEL_FRIEND"sv,
		"CMSG_SET_CONTACT_NOTES"sv,
		"CMSG_ADD_IGNORE"sv,
		"CMSG_DEL_IGNORE"sv,
		"CMSG_GROUP_INVITE"sv,
		"SMSG_GROUP_INVITE"sv,
		"CMSG_GROUP_CANCEL"sv,
		"SMSG_GROUP_CANCEL"sv,
		"CMSG_GROUP_ACCEPT"sv,
		"CMSG_GROUP_DECLINE"sv,
		"SMSG_GROUP_DECLINE"sv,
		"CMSG_GROUP_UNINVITE"sv,
		"CMSG_GROUP_UNINVITE_GUID"sv,
		"SMSG_GROUP_UNINVITE"sv,
		"CMSG_GROUP_SET_LEADER"sv,
		"SMSG_GROUP_SET_LEADER"sv,
		"CMSG_LOOT_METHOD"sv,
		"CMSG_GROUP_DISBAND"sv,
		"SMSG_GROUP_DESTROYED"sv,
		"SMSG_GROUP_LIST"sv,
		"SMSG_PARTY_MEMBER_STATS"sv,
		"SMSG_PARTY_COMMAND_RESULT"sv,
		"UMSG_UPDATE_GROUP_MEMBERS"sv,
		"CMSG_GUILD_CREATE"sv,
		"CMSG_GUILD_INVITE"sv,
		"SMSG_GUILD_INVITE"sv,
		"CMSG_GUILD_ACCEPT"sv,
		"CMSG_GUILD_DECLINE"sv,
		"SMSG_GUILD_DECLINE"sv,
		"CMSG_GUILD_INFO"sv,
		"SMSG_GUILD_INFO"sv,
		"CMSG_GUILD_ROSTER"sv,
		"SMSG_GUILD_ROSTER"sv,
		"CMSG_GUILD_PROMOTE"sv,
		"CMSG_GUILD_DEMOTE"sv,
		"CMSG_GUILD_LEAVE"sv,
		"CMSG_GUILD_REMOVE"sv,
		"CMSG_GUILD_DISBAND"sv,
		"CMSG_GUILD_LEADER"sv,
		"CMSG_GUILD_MOTD"sv,
		"SMSG_GUILD_EVENT"sv,
		"SMSG_GUILD_COMMAND_RESULT"sv,
		"UMSG_UPDATE_GUILD"sv,
		"CMSG_MESSAGECHAT"sv,
		"SMSG_MESSAGECHAT"sv,
		"CMSG_JOIN_CHANNEL"sv,
		"CMSG_LEAVE_CHANNEL"sv,
		"SMSG_CHANNEL_NOTIFY"sv,
		"CMSG_CHANNEL_LIST"sv,
		"SMSG_CHANNEL_LIST"sv,
		"CMSG_CHANNEL_PASSWORD"sv,
		"CMSG_CHANNEL_SET_OWNER"sv,
		"CMSG_CHANNEL_OWNER"sv,
		"CMSG_CHANNEL_MODERATOR"sv,
		"CMSG_CHANNEL_UNMODERATOR"sv,
		"CMSG_CHANNEL_MUTE"sv,
		"CMSG_CHANNEL_UNMUTE"sv,
		"CMSG_CHANNEL_INVITE"sv,
		"CMSG_CHANNEL_KICK"sv,
		"CMSG_CHANNEL_BAN"sv,
		"CMSG_CHANNEL_UNBAN"sv,
		"CMSG_CHANNEL_ANNOUNCEMENTS"sv,
		"CMSG_CHANNEL_MODERATE"sv,
		"SMSG_UPDATE_OBJECT"sv,
		"SMSG_DESTROY_OBJECT"sv,
		"CMSG_USE_ITEM"sv,
		"CMSG_OPEN_ITEM"sv,
		"CMSG_READ_ITEM"sv,
		"SMSG_READ_ITEM_OK"sv,
		"SMSG_READ_ITEM_FAILED"sv,
		"SMSG_ITEM_COOLDOWN"sv,
		"CMSG_GAMEOBJ_USE"sv,
		"CMSG_DESTROY_ITEMS"sv,
		"SMSG_GAMEOBJECT_CUSTOM_ANIM"sv,
		"CMSG_AREATRIGGER"sv,
		"MSG_MOVE_START_FORWARD"sv,
		"MSG_MOVE_START_BACKWARD"sv,
		"MSG_MOVE_STOP"sv,
		"MSG_MOVE_START_STRAFE_LEFT"sv,
		"MSG_MOVE_START_STRAFE_RIGHT"sv,
		"MSG_MOVE_STOP_STRAFE"sv,
		"MSG_MOVE_JUMP"sv,
		"MSG_MOVE_START_TURN_LEFT"sv,
		"MSG_MOVE_START_TURN_RIGHT"sv,
		"MSG_MOVE_STOP_TURN"sv,
		"MSG_MOVE_START_PITCH_UP"sv,
		"MSG_MOVE_START_PITCH_DOWN"sv,
		"MSG_MOVE_STOP_PITCH"sv,
		"MSG_MOVE_SET_RUN_MODE"sv,
		"MSG_MOVE_SET_WALK_MODE"sv,
		"MSG_MOVE_TOGGLE_LOGGING"sv,
		"MSG_MOVE_TELEPORT"sv,
		"MSG_MOVE_TELEPORT_CHEAT"sv,
		"MSG_MOVE_TELEPORT_ACK"sv,
		"MSG_MOVE_TOGGLE_FALL_LOGGING"sv,
		"MSG_MOVE_FALL_LAND"sv,
		"MSG_MOVE_START_SWIM"sv,
		"MSG_MOVE_STOP_SWIM"sv,
		"MSG_MOVE_SET_RUN_SPEED_CHEAT"sv,
		"MSG_MOVE_SET_RUN_SPEED"sv,
		"MSG_MOVE_SET_RUN_BACK_SPEED_CHEAT"sv,
		"MSG_MOVE_SET_RUN_BACK_SPEED"sv,
		"MSG_MOVE_SET_WALK_SPEED_CHEAT"sv,
		"MSG_MOVE_SET_WALK_SPEED"sv,
		"MSG_MOVE_SET_SWIM_SPEED_CHEAT"sv,
		"MSG_MOVE_SET_SWIM_SPEED"sv,
		"MSG_MOVE_SET_SWIM_BACK_SPEED_CHEAT"sv,
		"MSG_MOVE_SET_SWIM_BACK_SPEED"sv,
		"MSG_MOVE_SET_ALL_SPEED_CHEAT"sv,
		"MSG_MOVE_SET_TURN_RATE_CHEAT"sv,
		"MSG_MOVE_SET_TURN_RATE"sv,
		"MSG_MOVE_TOGGLE_COLLISION_CHEAT"sv,
		"MSG_MOVE_SET_FACING"sv,
		"MSG_MOVE_SET_PITCH"sv,
		"MSG_MOVE_WORLDPORT_ACK"sv,
		"SMSG_MONSTER_MOVE"sv,
		"SMSG_MOVE_WATER_WALK"sv,
		"SMSG_MOVE_LAND_WALK"sv,
		"CMSG_MOVE_CHARM_PORT_CHEAT"sv,
		"CMSG_MOVE_SET_RAW_POSITION"sv,
		"SMSG_FORCE_RUN_SPEED_CHANGE"sv,
		"CMSG_FORCE_RUN_SPEED_CHANGE_ACK"sv,
		"SMSG_FORCE_RUN_BACK_SPEED_CHANGE"sv,
		"CMSG_FORCE_RUN_BACK_SPEED_CHANGE_ACK"sv,
		"SMSG_FORCE_SWIM_SPEED_CHANGE"sv,
		"CMSG_FORCE_SWIM_SPEED_CHANGE_ACK"sv,
		"SMSG_FORCE_MOVE_ROOT"sv,
		"CMSG_FORCE_MOVE_ROOT_ACK"sv,
		"SMSG_FORCE_MOVE_UNROOT"sv,
		"CMSG_FORCE_MOVE_UNROOT_ACK"sv,
		"MSG_MOVE_ROOT"sv,
		"MSG_MOVE_UNROOT"sv,
		"MSG_MOVE_HEARTBEAT"sv,
		"SMSG_MOVE_KNOCK_BACK"sv,
		"CMSG_MOVE_KNOCK_BACK_ACK"sv,
		"MSG_MOVE_KNOCK_BACK"sv,
		"SMSG_MOVE_FEATHER_FALL"sv,
		"SMSG_MOVE_NORMAL_FALL"sv,
		"SMSG_MOVE_SET_HOVER"sv,
		"SMSG_MOVE_UNSET_HOVER"sv,
		"CMSG_MOVE_HOVER_ACK"sv,
		"MSG_MOVE_HOVER"sv,
		"CMSG_TRIGGER_CINEMATIC_CHEAT"sv,
		"CMSG_OPENING_CINEMATIC"sv,
		"SMSG_TRIGGER_CINEMATIC"sv,
		"CMSG_NEXT_CINEMATIC_CAMERA"sv,
		"CMSG_COMPLETE_CINEMATIC"sv,
		"SMSG_TUTORIAL_FLAGS"sv,
		"CMSG_TUTORIAL_FLAG"sv,
		"CMSG_TUTORIAL_CLEAR"sv,
		"CMSG_TUTORIAL_RESET"sv,
		"CMSG_STANDSTATECHANGE"sv,
		"CMSG_EMOTE"sv,
		"SMSG_EMOTE"sv,
		"CMSG_TEXT_EMOTE"sv,
		"SMSG_TEXT_EMOTE"sv,
		"CMSG_AUTOEQUIP_GROUND_ITEM"sv,
		"CMSG_AUTOSTORE_GROUND_ITEM"sv,
		"CMSG_AUTOSTORE_LOOT_ITEM"sv,
		"CMSG_STORE_LOOT_IN_SLOT"sv,
		"CMSG_AUTOEQUIP_ITEM"sv,
		"CMSG_AUTOSTORE_BAG_ITEM"sv,
		"CMSG_SWAP_ITEM"sv,
		"CMSG_SWAP_INV_ITEM"sv,
		"CMSG_SPLIT_ITEM"sv,
		"CMSG_AUTOEQUIP_ITEM_SLOT"sv,
		"CMSG_UNCLAIM_LICENSE"sv,
		"CMSG_DESTROYITEM"sv,
		"SMSG_INVENTORY_CHANGE_FAILURE"sv,
		"SMSG_OPEN_CONTAINER"sv,
		"CMSG_INSPECT"sv,
		"SMSG_INSPECT_RESULTS_UPDATE"sv,
		"CMSG_INITIATE_TRADE"sv,
		"CMSG_BEGIN_TRADE"sv,
		"CMSG_BUSY_TRADE"sv,
		"CMSG_IGNORE_TRADE"sv,
		"CMSG_ACCEPT_TRADE"sv,
		"CMSG_UNACCEPT_TRADE"sv,
		"CMSG_CANCEL_TRADE"sv,
		"CMSG_SET_TRADE_ITEM"sv,
		"CMSG_CLEAR_TRADE_ITEM"sv,
		"CMSG_SET_TRADE_GOLD"sv,
		"SMSG_TRADE_STATUS"sv,
		"SMSG_TRADE_STATUS_EXTENDED"sv,
		"SMSG_INITIALIZE_FACTIONS"sv,
		"SMSG_SET_FACTION_VISIBLE"sv,
		"SMSG_SET_FACTION_STANDING"sv,
		"CMSG_SET_FACTION_ATWAR"sv,
		"CMSG_SET_FACTION_CHEAT"sv,
		"SMSG_SET_PROFICIENCY"sv,
		"CMSG_SET_ACTION_BUTTON"sv,
		"SMSG_ACTION_BUTTONS"sv,
		"SMSG_INITIAL_SPELLS"sv,
		"SMSG_LEARNED_SPELL"sv,
		"SMSG_SUPERCEDED_SPELL"sv,
		"CMSG_NEW_SPELL_SLOT"sv,
		"CMSG_CAST_SPELL"sv,
		"CMSG_CANCEL_CAST"sv,
		"SMSG_CAST_FAILED"sv,
		"SMSG_SPELL_START"sv,
		"SMSG_SPELL_GO"sv,
		"SMSG_SPELL_FAILURE"sv,
		"SMSG_SPELL_COOLDOWN"sv,
		"SMSG_COOLDOWN_EVENT"sv,
		"CMSG_CANCEL_AURA"sv,
		"SMSG_EQUIPMENT_SET_ID"sv,
		"SMSG_PET_CAST_FAILED"sv,
		"MSG_CHANNEL_START"sv,
		"MSG_CHANNEL_UPDATE"sv,
		"CMSG_CANCEL_CHANNELLING"sv,
		"SMSG_AI_REACTION"sv,
		"CMSG_SET_SELECTION"sv,
		"CMSG_DELETEEQUIPMENT_SET"sv,
		"CMSG_INSTANCE_LOCK_RESPONSE"sv,
		"CMSG_DEBUG_PASSIVE_AURA"sv,
		"CMSG_ATTACKSWING"sv,
		"CMSG_ATTACKSTOP"sv,
		"SMSG_ATTACKSTART"sv,
		"SMSG_ATTACKSTOP"sv,
		"SMSG_ATTACKSWING_NOTINRANGE"sv,
		"SMSG_ATTACKSWING_BADFACING"sv,
		"SMSG_PENDING_RAID_LOCK"sv,
		"SMSG_ATTACKSWING_DEADTARGET"sv,
		"SMSG_ATTACKSWING_CANT_ATTACK"sv,
		"SMSG_ATTACKERSTATEUPDATE"sv,
		"SMSG_BATTLEFIELD_PORT_DENIED"sv,
		"CMSG_PERFORM_ACTION_SET"sv,
		"SMSG_RESUME_CAST_BAR"sv,
		"SMSG_CANCEL_COMBAT"sv,
		"SMSG_SPELLBREAKLOG"sv,
		"SMSG_SPELLHEALLOG"sv,
		"SMSG_SPELLENERGIZELOG"sv,
		"SMSG_BREAK_TARGET"sv,
		"CMSG_SAVE_PLAYER"sv,
		"CMSG_SETDEATHBINDPOINT"sv,
		"SMSG_BINDPOINTUPDATE"sv,
		"CMSG_GETDEATHBINDZONE"sv,
		"SMSG_BINDZONEREPLY"sv,
		"SMSG_PLAYERBOUND"sv,
		"SMSG_CLIENT_CONTROL_UPDATE"sv,
		"CMSG_REPOP_REQUEST"sv,
		"SMSG_RESURRECT_REQUEST"sv,
		"CMSG_RESURRECT_RESPONSE"sv,
		"CMSG_LOOT"sv,
		"CMSG_LOOT_MONEY"sv,
		"CMSG_LOOT_RELEASE"sv,
		"SMSG_LOOT_RESPONSE"sv,
		"SMSG_LOOT_RELEASE_RESPONSE"sv,
		"SMSG_LOOT_REMOVED"sv,
		"SMSG_LOOT_MONEY_NOTIFY"sv,
		"SMSG_LOOT_ITEM_NOTIFY"sv,
		"SMSG_LOOT_CLEAR_MONEY"sv,
		"SMSG_ITEM_PUSH_RESULT"sv,
		"SMSG_DUEL_REQUESTED"sv,
		"SMSG_DUEL_OUTOFBOUNDS"sv,
		"SMSG_DUEL_INBOUNDS"sv,
		"SMSG_DUEL_COMPLETE"sv,
		"SMSG_DUEL_WINNER"sv,
		"CMSG_DUEL_ACCEPTED"sv,
		"CMSG_DUEL_CANCELLED"sv,
		"SMSG_MOUNTRESULT"sv,
		"SMSG_DISMOUNTRESULT"sv,
		"SMSG_REMOVED_FROM_PVP_QUEUE"sv,
		"CMSG_MOUNTSPECIAL_ANIM"sv,
		"SMSG_MOUNTSPECIAL_ANIM"sv,
		"SMSG_PET_TAME_FAILURE"sv,
		"CMSG_PET_SET_ACTION"sv,
		"CMSG_PET_ACTION"sv,
		"CMSG_PET_ABANDON"sv,
		"CMSG_PET_RENAME"sv,
		"SMSG_PET_NAME_INVALID"sv,
		"SMSG_PET_SPELLS"sv,
		"SMSG_PET_MODE"sv,
		"CMSG_GOSSIP_HELLO"sv,
		"CMSG_GOSSIP_SELECT_OPTION"sv,
		"SMSG_GOSSIP_MESSAGE"sv,
		"SMSG_GOSSIP_COMPLETE"sv,
		"CMSG_NPC_TEXT_QUERY"sv,
		"SMSG_NPC_TEXT_UPDATE"sv,
		"SMSG_NPC_WONT_TALK"sv,
		"CMSG_QUESTGIVER_STATUS_QUERY"sv,
		"SMSG_QUESTGIVER_STATUS"sv,
		"CMSG_QUESTGIVER_HELLO"sv,
		"SMSG_QUESTGIVER_QUEST_LIST"sv,
		"CMSG_QUESTGIVER_QUERY_QUEST"sv,
		"CMSG_QUESTGIVER_QUEST_AUTOLAUNCH"sv,
		"SMSG_QUESTGIVER_QUEST_DETAILS"sv,
		"CMSG_QUESTGIVER_ACCEPT_QUEST"sv,
		"CMSG_QUESTGIVER_COMPLETE_QUEST"sv,
		"SMSG_QUESTGIVER_REQUEST_ITEMS"sv,
		"CMSG_QUESTGIVER_REQUEST_REWARD"sv,
		"SMSG_QUESTGIVER_OFFER_REWARD"sv,
		"CMSG_QUESTGIVER_CHOOSE_REWARD"sv,
		"SMSG_QUESTGIVER_QUEST_INVALID"sv,
		"CMSG_QUESTGIVER_CANCEL"sv,
		"SMSG_QUESTGIVER_QUEST_COMPLETE"sv,
		"SMSG_QUESTGIVER_QUEST_FAILED"sv,
		"CMSG_QUESTLOG_SWAP_QUEST"sv,
		"CMSG_QUESTLOG_REMOVE_QUEST"sv,
		"SMSG_QUESTLOG_FULL"sv,
		"SMSG_QUESTUPDATE_FAILED"sv,
		"SMSG_QUESTUPDATE_FAILEDTIMER"sv,
		"SMSG_QUESTUPDATE_COMPLETE"sv,
		"SMSG_QUESTUPDATE_ADD_KILL"sv,
		"SMSG_QUESTUPDATE_ADD_ITEM_OBOSLETE"sv,
		"CMSG_QUEST_CONFIRM_ACCEPT"sv,
		"SMSG_QUEST_CONFIRM_ACCEPT"sv,
		"CMSG_PUSHQUESTTOPARTY"sv,
		"CMSG_LIST_INVENTORY"sv,
		"SMSG_LIST_INVENTORY"sv,
		"CMSG_SELL_ITEM"sv,
		"SMSG_SELL_ITEM"sv,
		"CMSG_BUY_ITEM"sv,
		"CMSG_BUY_ITEM_IN_SLOT"sv,
		"SMSG_BUY_ITEM"sv,
		"SMSG_BUY_FAILED"sv,
		"CMSG_TAXICLEARALLNODES"sv,
		"CMSG_TAXIENABLEALLNODES"sv,
		"CMSG_TAXISHOWNODES"sv,
		"SMSG_SHOWTAXINODES"sv,
		"CMSG_TAXINODE_STATUS_QUERY"sv,
		"SMSG_TAXINODE_STATUS"sv,
		"CMSG_TAXIQUERYAVAILABLENODES"sv,
		"CMSG_ACTIVATETAXI"sv,
		"SMSG_ACTIVATETAXIREPLY"sv,
		"SMSG_NEW_TAXI_PATH"sv,
		"CMSG_TRAINER_LIST"sv,
		"SMSG_TRAINER_LIST"sv,
		"CMSG_TRAINER_BUY_SPELL"sv,
		"SMSG_TRAINER_BUY_SUCCEEDED"sv,
		"SMSG_TRAINER_BUY_FAILED"sv,
		"CMSG_BINDER_ACTIVATE"sv,
		"SMSG_PLAYERBINDERROR"sv,
		"CMSG_BANKER_ACTIVATE"sv,
		"SMSG_SHOW_BANK"sv,
		"CMSG_BUY_BANK_SLOT"sv,
		"SMSG_BUY_BANK_SLOT_RESULT"sv,
		"CMSG_PETITION_SHOWLIST"sv,
		"SMSG_PETITION_SHOWLIST"sv,
		"CMSG_PETITION_BUY"sv,
		"CMSG_PETITION_SHOW_SIGNATURES"sv,
		"SMSG_PETITION_SHOW_SIGNATURES"sv,
		"CMSG_PETITION_SIGN"sv,
		"SMSG_PETITION_SIGN_RESULTS"sv,
		"MSG_PETITION_DECLINE"sv,
		"CMSG_OFFER_PETITION"sv,
		"CMSG_TURN_IN_PETITION"sv,
		"SMSG_TURN_IN_PETITION_RESULTS"sv,
		"CMSG_PETITION_QUERY"sv,
		"SMSG_PETITION_QUERY_RESPONSE"sv,
		"SMSG_FISH_NOT_HOOKED"sv,
		"SMSG_FISH_ESCAPED"sv,
		"CMSG_BUG"sv,
		"SMSG_NOTIFICATION"sv,
		"CMSG_PLAYED_TIME"sv,
		"SMSG_PLAYED_TIME"sv,
		"CMSG_QUERY_TIME"sv,
		"SMSG_QUERY_TIME_RESPONSE"sv,
		"SMSG_LOG_XPGAIN"sv,
		"SMSG_AURACASTLOG"sv,
		"CMSG_RECLAIM_CORPSE"sv,
		"CMSG_WRAP_ITEM"sv,
		"SMSG_LEVELUP_INFO"sv,
		"MSG_MINIMAP_PING"sv,
		"SMSG_RESISTLOG"sv,
		"SMSG_ENCHANTMENTLOG"sv,
		"CMSG_SET_SKILL_CHEAT"sv,
		"SMSG_START_MIRROR_TIMER"sv,
		"SMSG_PAUSE_MIRROR_TIMER"sv,
		"SMSG_STOP_MIRROR_TIMER"sv,
		"CMSG_PING"sv,
		"SMSG_PONG"sv,
		"SMSG_CLEAR_COOLDOWN"sv,
		"SMSG_GAMEOBJECT_PAGETEXT"sv,
		"CMSG_SETSHEATHED"sv,
		"SMSG_COOLDOWN_CHEAT"sv,
		"SMSG_SPELL_DELAYED"sv,
		"CMSG_QUEST_POI_QUERY"sv,
		"SMSG_QUEST_POI_QUERY_RESPONSE"sv,
		"CMSG_GHOST"sv,
		"CMSG_GM_INVIS"sv,
		"SMSG_INVALID_PROMOTION_CODE"sv,
		"MSG_GM_BIND_OTHER"sv,
		"MSG_GM_SUMMON"sv,
		"SMSG_ITEM_TIME_UPDATE"sv,
		"SMSG_ITEM_ENCHANT_TIME_UPDATE"sv,
		"SMSG_AUTH_CHALLENGE"sv,
		"CMSG_AUTH_SESSION"sv,
		"SMSG_AUTH_RESPONSE"sv,
		"MSG_GM_SHOWLABEL"sv,
		"CMSG_PET_CAST_SPELL"sv,
		"MSG_SAVE_GUILD_EMBLEM"sv,
		"MSG_TABARDVENDOR_ACTIVATE"sv,
		"SMSG_PLAY_SPELL_VISUAL"sv,
		"CMSG_ZONEUPDATE"sv,
		"SMSG_PARTYKILLLOG"sv,
		"SMSG_COMPRESSED_UPDATE_OBJECT"sv,
		"SMSG_PLAY_SPELL_IMPACT"sv,
		"SMSG_EXPLORATION_EXPERIENCE"sv,
		"CMSG_GM_SET_SECURITY_GROUP"sv,
		"CMSG_GM_NUKE"sv,
		"MSG_RANDOM_ROLL"sv,
		"SMSG_ENVIRONMENTALDAMAGELOG"sv,
		"CMSG_CHANGEPLAYER_DIFFICULTY"sv,
		"SMSG_RWHOIS"sv,
		"SMSG_LFG_PLAYER_REWARD"sv,
		"SMSG_LFG_TELEPORT_DENIED"sv,
		"CMSG_UNLEARN_SPELL"sv,
		"CMSG_UNLEARN_SKILL"sv,
		"SMSG_REMOVED_SPELL"sv,
		"CMSG_DECHARGE"sv,
		"CMSG_GMTICKET_CREATE"sv,
		"SMSG_GMTICKET_CREATE"sv,
		"CMSG_GMTICKET_UPDATETEXT"sv,
		"SMSG_GMTICKET_UPDATETEXT"sv,
		"SMSG_ACCOUNT_DATA_TIMES"sv,
		"CMSG_REQUEST_ACCOUNT_DATA"sv,
		"CMSG_UPDATE_ACCOUNT_DATA"sv,
		"SMSG_UPDATE_ACCOUNT_DATA"sv,
		"SMSG_CLEAR_FAR_SIGHT_IMMEDIATE"sv,
		"SMSG_CHANGEPLAYER_DIFFICULTY_RESULT"sv,
		"CMSG_GM_TEACH"sv,
		"CMSG_GM_CREATE_ITEM_TARGET"sv,
		"CMSG_GMTICKET_GETTICKET"sv,
		"SMSG_GMTICKET_GETTICKET"sv,
		"CMSG_UNLEARN_TALENTS"sv,
		"SMSG_INSTANCE_ENCOUNTER"sv,
		"SMSG_GAMEOBJECT_DESPAWN_ANIM"sv,
		"MSG_CORPSE_QUERY"sv,
		"CMSG_GMTICKET_DELETETICKET"sv,
		"SMSG_GMTICKET_DELETETICKET"sv,
		"SMSG_CHAT_WRONG_FACTION"sv,
		"CMSG_GMTICKET_SYSTEMSTATUS"sv,
		"SMSG_GMTICKET_SYSTEMSTATUS"sv,
		"CMSG_SPIRIT_HEALER_ACTIVATE"sv,
		"CMSG_SET_STAT_CHEAT"sv,
		"SMSG_QUEST_FORCE_REMOVED"sv,
		"CMSG_SKILL_BUY_STEP"sv,
		"CMSG_SKILL_BUY_RANK"sv,
		"CMSG_XP_CHEAT"sv,
		"SMSG_SPIRIT_HEALER_CONFIRM"sv,
		"CMSG_CHARACTER_POINT_CHEAT"sv,
		"SMSG_GOSSIP_POI"sv,
		"CMSG_CHAT_IGNORED"sv,
		"CMSG_GM_VISION"sv,
		"CMSG_SERVER_COMMAND"sv,
		"CMSG_GM_SILENCE"sv,
		"CMSG_GM_REVEALTO"sv,
		"CMSG_GM_RESURRECT"sv,
		"CMSG_GM_SUMMONMOB"sv,
		"CMSG_GM_MOVECORPSE"sv,
		"CMSG_GM_FREEZE"sv,
		"CMSG_GM_UBERINVIS"sv,
		"CMSG_GM_REQUEST_PLAYER_INFO"sv,
		"SMSG_GM_PLAYER_INFO"sv,
		"CMSG_GUILD_RANK"sv,
		"CMSG_GUILD_ADD_RANK"sv,
		"CMSG_GUILD_DEL_RANK"sv,
		"CMSG_GUILD_SET_PUBLIC_NOTE"sv,
		"CMSG_GUILD_SET_OFFICER_NOTE"sv,
		"SMSG_LOGIN_VERIFY_WORLD"sv,
		"CMSG_CLEAR_EXPLORATION"sv,
		"CMSG_SEND_MAIL"sv,
		"SMSG_SEND_MAIL_RESULT"sv,
		"CMSG_GET_MAIL_LIST"sv,
		"SMSG_MAIL_LIST_RESULT"sv,
		"CMSG_BATTLEFIELD_LIST"sv,
		"SMSG_BATTLEFIELD_LIST"sv,
		"CMSG_BATTLEFIELD_JOIN"sv,
		"SMSG_FORCE_SET_VEHICLE_REC_ID"sv,
		"CMSG_SET_VEHICLE_REC_ID_ACK"sv,
		"CMSG_TAXICLEARNODE"sv,
		"CMSG_TAXIENABLENODE"sv,
		"CMSG_ITEM_TEXT_QUERY"sv,
		"SMSG_ITEM_TEXT_QUERY_RESPONSE"sv,
		"CMSG_MAIL_TAKE_MONEY"sv,
		"CMSG_MAIL_TAKE_ITEM"sv,
		"CMSG_MAIL_MARK_AS_READ"sv,
		"CMSG_MAIL_RETURN_TO_SENDER"sv,
		"CMSG_MAIL_DELETE"sv,
		"CMSG_MAIL_CREATE_TEXT_ITEM"sv,
		"SMSG_SPELLLOGMISS"sv,
		"SMSG_SPELLLOGEXECUTE"sv,
		"SMSG_DEBUGAURAPROC"sv,
		"SMSG_PERIODICAURALOG"sv,
		"SMSG_SPELLDAMAGESHIELD"sv,
		"SMSG_SPELLNONMELEEDAMAGELOG"sv,
		"CMSG_LEARN_TALENT"sv,
		"SMSG_RESURRECT_FAILED"sv,
		"CMSG_TOGGLE_PVP"sv,
		"SMSG_ZONE_UNDER_ATTACK"sv,
		"MSG_AUCTION_HELLO"sv,
		"CMSG_AUCTION_SELL_ITEM"sv,
		"CMSG_AUCTION_REMOVE_ITEM"sv,
		"CMSG_AUCTION_LIST_ITEMS"sv,
		"CMSG_AUCTION_LIST_OWNER_ITEMS"sv,
		"CMSG_AUCTION_PLACE_BID"sv,
		"SMSG_AUCTION_COMMAND_RESULT"sv,
		"SMSG_AUCTION_LIST_RESULT"sv,
		"SMSG_AUCTION_OWNER_LIST_RESULT"sv,
		"SMSG_AUCTION_BIDDER_NOTIFICATION"sv,
		"SMSG_AUCTION_OWNER_NOTIFICATION"sv,
		"SMSG_PROCRESIST"sv,
		"SMSG_COMBAT_EVENT_FAILED"sv,
		"SMSG_DISPEL_FAILED"sv,
		"SMSG_SPELLORDAMAGE_IMMUNE"sv,
		"CMSG_AUCTION_LIST_BIDDER_ITEMS"sv,
		"SMSG_AUCTION_BIDDER_LIST_RESULT"sv,
		"SMSG_SET_FLAT_SPELL_MODIFIER"sv,
		"SMSG_SET_PCT_SPELL_MODIFIER"sv,
		"CMSG_SET_AMMO"sv,
		"SMSG_CORPSE_RECLAIM_DELAY"sv,
		"CMSG_SET_ACTIVE_MOVER"sv,
		"CMSG_PET_CANCEL_AURA"sv,
		"CMSG_PLAYER_AI_CHEAT"sv,
		"CMSG_CANCEL_AUTO_REPEAT_SPELL"sv,
		"MSG_GM_ACCOUNT_ONLINE"sv,
		"MSG_LIST_STABLED_PETS"sv,
		"CMSG_STABLE_PET"sv,
		"CMSG_UNSTABLE_PET"sv,
		"CMSG_BUY_STABLE_SLOT"sv,
		"SMSG_STABLE_RESULT"sv,
		"CMSG_STABLE_REVIVE_PET"sv,
		"CMSG_STABLE_SWAP_PET"sv,
		"MSG_QUEST_PUSH_RESULT"sv,
		"SMSG_PLAY_MUSIC"sv,
		"SMSG_PLAY_OBJECT_SOUND"sv,
		"CMSG_REQUEST_PET_INFO"sv,
		"CMSG_FAR_SIGHT"sv,
		"SMSG_SPELLDISPELLOG"sv,
		"SMSG_DAMAGE_CALC_LOG"sv,
		"CMSG_ENABLE_DAMAGE_LOG"sv,
		"CMSG_GROUP_CHANGE_SUB_GROUP"sv,
		"CMSG_REQUEST_PARTY_MEMBER_STATS"sv,
		"CMSG_GROUP_SWAP_SUB_GROUP"sv,
		"CMSG_RESET_FACTION_CHEAT"sv,
		"CMSG_AUTOSTORE_BANK_ITEM"sv,
		"CMSG_AUTOBANK_ITEM"sv,
		"MSG_QUERY_NEXT_MAIL_TIME"sv,
		"SMSG_RECEIVED_MAIL"sv,
		"SMSG_RAID_GROUP_ONLY"sv,
		"CMSG_SET_DURABILITY_CHEAT"sv,
		"CMSG_SET_PVP_RANK_CHEAT"sv,
		"CMSG_ADD_PVP_MEDAL_CHEAT"sv,
		"CMSG_DEL_PVP_MEDAL_CHEAT"sv,
		"CMSG_SET_PVP_TITLE"sv,
		"SMSG_PVP_CREDIT"sv,
		"SMSG_AUCTION_REMOVED_NOTIFICATION"sv,
		"CMSG_GROUP_RAID_CONVERT"sv,
		"CMSG_GROUP_ASSISTANT_LEADER"sv,
		"CMSG_BUYBACK_ITEM"sv,
		"SMSG_SERVER_MESSAGE"sv,
		"CMSG_SET_SAVED_INSTANCE_EXTEND"sv,
		"SMSG_LFG_OFFER_CONTINUE"sv,
		"CMSG_TEST_DROP_RATE"sv,
		"SMSG_TEST_DROP_RATE_RESULT"sv,
		"CMSG_LFG_GET_STATUS"sv,
		"SMSG_SHOW_MAILBOX"sv,
		"SMSG_RESET_RANGED_COMBAT_TIMER"sv,
		"SMSG_CHAT_NOT_IN_PARTY"sv,
		"CMSG_GMTICKETSYSTEM_TOGGLE"sv,
		"CMSG_CANCEL_GROWTH_AURA"sv,
		"SMSG_CANCEL_AUTO_REPEAT"sv,
		"SMSG_STANDSTATE_UPDATE"sv,
		"SMSG_LOOT_ALL_PASSED"sv,
		"SMSG_LOOT_ROLL_WON"sv,
		"CMSG_LOOT_ROLL"sv,
		"SMSG_LOOT_START_ROLL"sv,
		"SMSG_LOOT_ROLL"sv,
		"CMSG_LOOT_MASTER_GIVE"sv,
		"SMSG_LOOT_MASTER_LIST"sv,
		"SMSG_SET_FORCED_REACTIONS"sv,
		"SMSG_SPELL_FAILED_OTHER"sv,
		"SMSG_GAMEOBJECT_RESET_STATE"sv,
		"CMSG_REPAIR_ITEM"sv,
		"SMSG_CHAT_PLAYER_NOT_FOUND"sv,
		"MSG_TALENT_WIPE_CONFIRM"sv,
		"SMSG_SUMMON_REQUEST"sv,
		"CMSG_SUMMON_RESPONSE"sv,
		"MSG_DEV_SHOWLABEL"sv,
		"SMSG_MONSTER_MOVE_TRANSPORT"sv,
		"SMSG_PET_BROKEN"sv,
		"MSG_MOVE_FEATHER_FALL"sv,
		"MSG_MOVE_WATER_WALK"sv,
		"CMSG_SERVER_BROADCAST"sv,
		"CMSG_SELF_RES"sv,
		"SMSG_FEIGN_DEATH_RESISTED"sv,
		"CMSG_RUN_SCRIPT"sv,
		"SMSG_SCRIPT_MESSAGE"sv,
		"SMSG_DUEL_COUNTDOWN"sv,
		"SMSG_AREA_TRIGGER_MESSAGE"sv,
		"CMSG_SHOWING_HELM"sv,
		"CMSG_SHOWING_CLOAK"sv,
		"SMSG_ROLE_CHOSEN"sv,
		"SMSG_PLAYER_SKINNED"sv,
		"SMSG_DURABILITY_DAMAGE_DEATH"sv,
		"CMSG_SET_EXPLORATION"sv,
		"CMSG_SET_ACTIONBAR_TOGGLES"sv,
		"UMSG_DELETE_GUILD_CHARTER"sv,
		"MSG_PETITION_RENAME"sv,
		"SMSG_INIT_WORLD_STATES"sv,
		"SMSG_UPDATE_WORLD_STATE"sv,
		"CMSG_ITEM_NAME_QUERY"sv,
		"SMSG_ITEM_NAME_QUERY_RESPONSE"sv,
		"SMSG_PET_ACTION_FEEDBACK"sv,
		"CMSG_CHAR_RENAME"sv,
		"SMSG_CHAR_RENAME"sv,
		"CMSG_MOVE_SPLINE_DONE"sv,
		"CMSG_MOVE_FALL_RESET"sv,
		"SMSG_INSTANCE_SAVE_CREATED"sv,
		"SMSG_RAID_INSTANCE_INFO"sv,
		"CMSG_REQUEST_RAID_INFO"sv,
		"CMSG_MOVE_TIME_SKIPPED"sv,
		"CMSG_MOVE_FEATHER_FALL_ACK"sv,
		"CMSG_MOVE_WATER_WALK_ACK"sv,
		"CMSG_MOVE_NOT_ACTIVE_MOVER"sv,
		"SMSG_PLAY_SOUND"sv,
		"CMSG_BATTLEFIELD_STATUS"sv,
		"SMSG_BATTLEFIELD_STATUS"sv,
		"CMSG_BATTLEFIELD_PORT"sv,
		"MSG_INSPECT_HONOR_STATS"sv,
		"CMSG_BATTLEMASTER_HELLO"sv,
		"CMSG_MOVE_START_SWIM_CHEAT"sv,
		"CMSG_MOVE_STOP_SWIM_CHEAT"sv,
		"SMSG_FORCE_WALK_SPEED_CHANGE"sv,
		"CMSG_FORCE_WALK_SPEED_CHANGE_ACK"sv,
		"SMSG_FORCE_SWIM_BACK_SPEED_CHANGE"sv,
		"CMSG_FORCE_SWIM_BACK_SPEED_CHANGE_ACK"sv,
		"SMSG_FORCE_TURN_RATE_CHANGE"sv,
		"CMSG_FORCE_TURN_RATE_CHANGE_ACK"sv,
		"MSG_PVP_LOG_DATA"sv,
		"CMSG_LEAVE_BATTLEFIELD"sv,
		"CMSG_AREA_SPIRIT_HEALER_QUERY"sv,
		"CMSG_AREA_SPIRIT_HEALER_QUEUE"sv,
		"SMSG_AREA_SPIRIT_HEALER_TIME"sv,
		"CMSG_GM_UNTEACH"sv,
		"SMSG_WARDEN_DATA"sv,
		"CMSG_WARDEN_DATA"sv,
		"SMSG_GROUP_JOINED_BATTLEGROUND"sv,
		"MSG_BATTLEGROUND_PLAYER_POSITIONS"sv,
		"CMSG_PET_STOP_ATTACK"sv,
		"SMSG_BINDER_CONFIRM"sv,
		"SMSG_BATTLEGROUND_PLAYER_JOINED"sv,
		"SMSG_BATTLEGROUND_PLAYER_LEFT"sv,
		"CMSG_BATTLEMASTER_JOIN"sv,
		"SMSG_ADDON_INFO"sv,
		"CMSG_PET_UNLEARN"sv,
		"SMSG_PET_UNLEARN_CONFIRM"sv,
		"SMSG_PARTY_MEMBER_STATS_FULL"sv,
		"CMSG_PET_SPELL_AUTOCAST"sv,
		"SMSG_WEATHER"sv,
		"SMSG_PLAY_TIME_WARNING"sv,
		"SMSG_MINIGAME_SETUP"sv,
		"SMSG_MINIGAME_STATE"sv,
		"CMSG_MINIGAME_MOVE"sv,
		"SMSG_MINIGAME_MOVE_FAILED"sv,
		"SMSG_RAID_INSTANCE_MESSAGE"sv,
		"SMSG_COMPRESSED_MOVES"sv,
		"CMSG_GUILD_INFO_TEXT"sv,
		"SMSG_CHAT_RESTRICTED"sv,
		"SMSG_SPLINE_SET_RUN_SPEED"sv,
		"SMSG_SPLINE_SET_RUN_BACK_SPEED"sv,
		"SMSG_SPLINE_SET_SWIM_SPEED"sv,
		"SMSG_SPLINE_SET_WALK_SPEED"sv,
		"SMSG_SPLINE_SET_SWIM_BACK_SPEED"sv,
		"SMSG_SPLINE_SET_TURN_RATE"sv,
		"SMSG_SPLINE_MOVE_UNROOT"sv,
		"SMSG_SPLINE_MOVE_FEATHER_FALL"sv,
		"SMSG_SPLINE_MOVE_NORMAL_FALL"sv,
		"SMSG_SPLINE_MOVE_SET_HOVER"sv,
		"SMSG_SPLINE_MOVE_UNSET_HOVER"sv,
		"SMSG_SPLINE_MOVE_WATER_WALK"sv,
		"SMSG_SPLINE_MOVE_LAND_WALK"sv,
		"SMSG_SPLINE_MOVE_START_SWIM"sv,
		"SMSG_SPLINE_MOVE_STOP_SWIM"sv,
		"SMSG_SPLINE_MOVE_SET_RUN_MODE"sv,
		"SMSG_SPLINE_MOVE_SET_WALK_MODE"sv,
		"CMSG_GM_NUKE_ACCOUNT"sv,
		"MSG_GM_DESTROY_CORPSE"sv,
		"CMSG_GM_DESTROY_ONLINE_CORPSE"sv,
		"CMSG_ACTIVATETAXIEXPRESS"sv,
		"SMSG_SET_FACTION_ATWAR"sv,
		"SMSG_GAMETIMEBIAS_SET"sv,
		"CMSG_DEBUG_ACTIONS_START"sv,
		"CMSG_DEBUG_ACTIONS_STOP"sv,
		"CMSG_SET_FACTION_INACTIVE"sv,
		"CMSG_SET_WATCHED_FACTION"sv,
		"MSG_MOVE_TIME_SKIPPED"sv,
		"SMSG_SPLINE_MOVE_ROOT"sv,
		"CMSG_SET_EXPLORATION_ALL"sv,
		"SMSG_INVALIDATE_PLAYER"sv,
		"CMSG_RESET_INSTANCES"sv,
		"SMSG_INSTANCE_RESET"sv,
		"SMSG_INSTANCE_RESET_FAILED"sv,
		"SMSG_UPDATE_LAST_INSTANCE"sv,
		"MSG_RAID_TARGET_UPDATE"sv,
		"MSG_RAID_READY_CHECK"sv,
		"CMSG_LUA_USAGE"sv,
		"SMSG_PET_ACTION_SOUND"sv,
		"SMSG_PET_DISMISS_SOUND"sv,
		"SMSG_GHOSTEE_GONE"sv,
		"CMSG_GM_UPDATE_TICKET_STATUS"sv,
		"SMSG_GM_TICKET_STATUS_UPDATE"sv,
		"MSG_SET_DUNGEON_DIFFICULTY"sv,
		"CMSG_GMSURVEY_SUBMIT"sv,
		"SMSG_UPDATE_INSTANCE_OWNERSHIP"sv,
		"CMSG_IGNORE_KNOCKBACK_CHEAT"sv,
		"SMSG_CHAT_PLAYER_AMBIGUOUS"sv,
		"MSG_DELAY_GHOST_TELEPORT"sv,
		"SMSG_SPELLINSTAKILLLOG"sv,
		"SMSG_SPELL_UPDATE_CHAIN_TARGETS"sv,
		"CMSG_CHAT_FILTERED"sv,
		"SMSG_EXPECTED_SPAM_RECORDS"sv,
		"SMSG_SPELLSTEALLOG"sv,
		"CMSG_LOTTERY_QUERY_OBSOLETE"sv,
		"SMSG_LOTTERY_QUERY_RESULT_OBSOLETE"sv,
		"CMSG_BUY_LOTTERY_TICKET_OBSOLETE"sv,
		"SMSG_LOTTERY_RESULT_OBSOLETE"sv,
		"SMSG_CHARACTER_PROFILE"sv,
		"SMSG_CHARACTER_PROFILE_REALM_CONNECTED"sv,
		"SMSG_DEFENSE_MESSAGE"sv,
		"SMSG_INSTANCE_DIFFICULTY"sv,
		"MSG_GM_RESETINSTANCELIMIT"sv,
		"SMSG_MOTD"sv,
		"SMSG_MOVE_SET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY"sv,
		"SMSG_MOVE_UNSET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY"sv,
		"CMSG_MOVE_SET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY_ACK"sv,
		"MSG_MOVE_START_SWIM_CHEAT"sv,
		"MSG_MOVE_STOP_SWIM_CHEAT"sv,
		"SMSG_MOVE_SET_CAN_FLY"sv,
		"SMSG_MOVE_UNSET_CAN_FLY"sv,
		"CMSG_MOVE_SET_CAN_FLY_ACK"sv,
		"CMSG_MOVE_SET_FLY"sv,
		"CMSG_SOCKET_GEMS"sv,
		"CMSG_ARENA_TEAM_CREATE"sv,
		"SMSG_ARENA_TEAM_COMMAND_RESULT"sv,
		"MSG_MOVE_UPDATE_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY"sv,
		"CMSG_ARENA_TEAM_QUERY"sv,
		"SMSG_ARENA_TEAM_QUERY_RESPONSE"sv,
		"CMSG_ARENA_TEAM_ROSTER"sv,
		"SMSG_ARENA_TEAM_ROSTER"sv,
		"CMSG_ARENA_TEAM_INVITE"sv,
		"SMSG_ARENA_TEAM_INVITE"sv,
		"CMSG_ARENA_TEAM_ACCEPT"sv,
		"CMSG_ARENA_TEAM_DECLINE"sv,
		"CMSG_ARENA_TEAM_LEAVE"sv,
		"CMSG_ARENA_TEAM_REMOVE"sv,
		"CMSG_ARENA_TEAM_DISBAND"sv,
		"CMSG_ARENA_TEAM_LEADER"sv,
		"SMSG_ARENA_TEAM_EVENT"sv,
		"CMSG_BATTLEMASTER_JOIN_ARENA"sv,
		"MSG_MOVE_START_ASCEND"sv,
		"MSG_MOVE_STOP_ASCEND"sv,
		"SMSG_ARENA_TEAM_STATS"sv,
		"CMSG_LFG_JOIN"sv,
		"CMSG_LFG_LEAVE"sv,
		"CMSG_LFG_SEARCH_JOIN"sv,
		"CMSG_LFG_SEARCH_LEAVE"sv,
		"SMSG_LFG_SEARCH_RESULTS"sv,
		"SMSG_LFG_PROPOSAL_UPDATE"sv,
		"CMSG_LFG_PROPOSAL_RESPONSE"sv,
		"SMSG_LFG_ROLE_CHECK_UPDATE"sv,
		"SMSG_LFG_JOIN_RESULT"sv,
		"SMSG_LFG_QUEUE_STATUS"sv,
		"CMSG_SET_LFG_COMMENT"sv,
		"SMSG_LFG_UPDATE_PLAYER"sv,
		"SMSG_LFG_UPDATE_PARTY"sv,
		"SMSG_LFG_UPDATE_SEARCH"sv,
		"CMSG_LFG_SET_ROLES"sv,
		"CMSG_LFG_SET_NEEDS"sv,
		"CMSG_LFG_BOOT_PLAYER_VOTE"sv,
		"SMSG_LFG_BOOT_PLAYER"sv,
		"CMSG_LFG_GET_PLAYER_INFO"sv,
		"SMSG_LFG_PLAYER_INFO"sv,
		"CMSG_LFG_TELEPORT"sv,
		"CMSG_LFG_GET_PARTY_INFO"sv,
		"SMSG_LFG_PARTY_INFO"sv,
		"SMSG_TITLE_EARNED"sv,
		"CMSG_SET_TITLE"sv,
		"CMSG_CANCEL_MOUNT_AURA"sv,
		"SMSG_ARENA_ERROR"sv,
		"MSG_INSPECT_ARENA_TEAMS"sv,
		"SMSG_DEATH_RELEASE_LOC"sv,
		"CMSG_CANCEL_TEMP_ENCHANTMENT"sv,
		"SMSG_FORCED_DEATH_UPDATE"sv,
		"CMSG_CHEAT_SET_HONOR_CURRENCY"sv,
		"CMSG_CHEAT_SET_ARENA_CURRENCY"sv,
		"MSG_MOVE_SET_FLIGHT_SPEED_CHEAT"sv,
		"MSG_MOVE_SET_FLIGHT_SPEED"sv,
		"MSG_MOVE_SET_FLIGHT_BACK_SPEED_CHEAT"sv,
		"MSG_MOVE_SET_FLIGHT_BACK_SPEED"sv,
		"SMSG_FORCE_FLIGHT_SPEED_CHANGE"sv,
		"CMSG_FORCE_FLIGHT_SPEED_CHANGE_ACK"sv,
		"SMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE"sv,
		"CMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE_ACK"sv,
		"SMSG_SPLINE_SET_FLIGHT_SPEED"sv,
		"SMSG_SPLINE_SET_FLIGHT_BACK_SPEED"sv,
		"CMSG_MAELSTROM_INVALIDATE_CACHE"sv,
		"SMSG_FLIGHT_SPLINE_SYNC"sv,
		"CMSG_SET_TAXI_BENCHMARK_MODE"sv,
		"SMSG_JOINED_BATTLEGROUND_QUEUE"sv,
		"SMSG_REALM_SPLIT"sv,
		"CMSG_REALM_SPLIT"sv,
		"CMSG_MOVE_CHNG_TRANSPORT"sv,
		"MSG_PARTY_ASSIGNMENT"sv,
		"SMSG_OFFER_PETITION_ERROR"sv,
		"SMSG_TIME_SYNC_REQ"sv,
		"CMSG_TIME_SYNC_RESP"sv,
		"CMSG_SEND_LOCAL_EVENT"sv,
		"CMSG_SEND_GENERAL_TRIGGER"sv,
		"CMSG_SEND_COMBAT_TRIGGER"sv,
		"CMSG_MAELSTROM_GM_SENT_MAIL"sv,
		"SMSG_RESET_FAILED_NOTIFY"sv,
		"SMSG_REAL_GROUP_UPDATE"sv,
		"SMSG_LFG_DISABLED"sv,
		"CMSG_ACTIVE_PVP_CHEAT"sv,
		"CMSG_CHEAT_DUMP_ITEMS_DEBUG_ONLY"sv,
		"SMSG_CHEAT_DUMP_ITEMS_DEBUG_ONLY_RESPONSE"sv,
		"SMSG_CHEAT_DUMP_ITEMS_DEBUG_ONLY_RESPONSE_WRITE_FILE"sv,
		"SMSG_UPDATE_COMBO_POINTS"sv,
		"SMSG_VOICE_SESSION_ROSTER_UPDATE"sv,
		"SMSG_VOICE_SESSION_LEAVE"sv,
		"SMSG_VOICE_SESSION_ADJUST_PRIORITY"sv,
		"CMSG_VOICE_SET_TALKER_MUTED_REQUEST"sv,
		"SMSG_VOICE_SET_TALKER_MUTED"sv,
		"SMSG_INIT_EXTRA_AURA_INFO_OBSOLETE"sv,
		"SMSG_SET_EXTRA_AURA_INFO_OBSOLETE"sv,
		"SMSG_SET_EXTRA_AURA_INFO_NEED_UPDATE_OBSOLETE"sv,
		"SMSG_CLEAR_EXTRA_AURA_INFO_OBSOLETE"sv,
		"MSG_MOVE_START_DESCEND"sv,
		"CMSG_IGNORE_REQUIREMENTS_CHEAT"sv,
		"SMSG_IGNORE_REQUIREMENTS_CHEAT"sv,
		"SMSG_SPELL_CHANCE_PROC_LOG"sv,
		"CMSG_MOVE_SET_RUN_SPEED"sv,
		"SMSG_DISMOUNT"sv,
		"MSG_MOVE_UPDATE_CAN_FLY"sv,
		"MSG_RAID_READY_CHECK_CONFIRM"sv,
		"CMSG_VOICE_SESSION_ENABLE"sv,
		"SMSG_VOICE_SESSION_ENABLE"sv,
		"SMSG_VOICE_PARENTAL_CONTROLS"sv,
		"CMSG_GM_WHISPER"sv,
		"SMSG_GM_MESSAGECHAT"sv,
		"MSG_GM_GEARRATING"sv,
		"CMSG_COMMENTATOR_ENABLE"sv,
		"SMSG_COMMENTATOR_STATE_CHANGED"sv,
		"CMSG_COMMENTATOR_GET_MAP_INFO"sv,
		"SMSG_COMMENTATOR_MAP_INFO"sv,
		"CMSG_COMMENTATOR_GET_PLAYER_INFO"sv,
		"SMSG_COMMENTATOR_GET_PLAYER_INFO"sv,
		"SMSG_COMMENTATOR_PLAYER_INFO"sv,
		"CMSG_COMMENTATOR_ENTER_INSTANCE"sv,
		"CMSG_COMMENTATOR_EXIT_INSTANCE"sv,
		"CMSG_COMMENTATOR_INSTANCE_COMMAND"sv,
		"SMSG_CLEAR_TARGET"sv,
		"CMSG_BOT_DETECTED"sv,
		"SMSG_CROSSED_INEBRIATION_THRESHOLD"sv,
		"CMSG_CHEAT_PLAYER_LOGIN"sv,
		"CMSG_CHEAT_PLAYER_LOOKUP"sv,
		"SMSG_CHEAT_PLAYER_LOOKUP"sv,
		"SMSG_KICK_REASON"sv,
		"MSG_RAID_READY_CHECK_FINISHED"sv,
		"CMSG_COMPLAIN"sv,
		"SMSG_COMPLAIN_RESULT"sv,
		"SMSG_FEATURE_SYSTEM_STATUS"sv,
		"CMSG_GM_SHOW_COMPLAINTS"sv,
		"CMSG_GM_UNSQUELCH"sv,
		"CMSG_CHANNEL_SILENCE_VOICE"sv,
		"CMSG_CHANNEL_SILENCE_ALL"sv,
		"CMSG_CHANNEL_UNSILENCE_VOICE"sv,
		"CMSG_CHANNEL_UNSILENCE_ALL"sv,
		"CMSG_TARGET_CAST"sv,
		"CMSG_TARGET_SCRIPT_CAST"sv,
		"CMSG_CHANNEL_DISPLAY_LIST"sv,
		"CMSG_SET_ACTIVE_VOICE_CHANNEL"sv,
		"CMSG_GET_CHANNEL_MEMBER_COUNT"sv,
		"SMSG_CHANNEL_MEMBER_COUNT"sv,
		"CMSG_CHANNEL_VOICE_ON"sv,
		"CMSG_CHANNEL_VOICE_OFF"sv,
		"CMSG_DEBUG_LIST_TARGETS"sv,
		"SMSG_DEBUG_LIST_TARGETS"sv,
		"SMSG_AVAILABLE_VOICE_CHANNEL"sv,
		"CMSG_ADD_VOICE_IGNORE"sv,
		"CMSG_DEL_VOICE_IGNORE"sv,
		"CMSG_PARTY_SILENCE"sv,
		"CMSG_PARTY_UNSILENCE"sv,
		"MSG_NOTIFY_PARTY_SQUELCH"sv,
		"SMSG_COMSAT_RECONNECT_TRY"sv,
		"SMSG_COMSAT_DISCONNECT"sv,
		"SMSG_COMSAT_CONNECT_FAIL"sv,
		"SMSG_VOICE_CHAT_STATUS"sv,
		"CMSG_REPORT_PVP_AFK"sv,
		"SMSG_REPORT_PVP_AFK_RESULT"sv,
		"CMSG_GUILD_BANKER_ACTIVATE"sv,
		"CMSG_GUILD_BANK_QUERY_TAB"sv,
		"SMSG_GUILD_BANK_LIST"sv,
		"CMSG_GUILD_BANK_SWAP_ITEMS"sv,
		"CMSG_GUILD_BANK_BUY_TAB"sv,
		"CMSG_GUILD_BANK_UPDATE_TAB"sv,
		"CMSG_GUILD_BANK_DEPOSIT_MONEY"sv,
		"CMSG_GUILD_BANK_WITHDRAW_MONEY"sv,
		"MSG_GUILD_BANK_LOG_QUERY"sv,
		"CMSG_SET_CHANNEL_WATCH"sv,
		"SMSG_USERLIST_ADD"sv,
		"SMSG_USERLIST_REMOVE"sv,
		"SMSG_USERLIST_UPDATE"sv,
		"CMSG_CLEAR_CHANNEL_WATCH"sv,
		"SMSG_INSPECT_RESULTS"sv,
		"SMSG_GOGOGO_OBSOLETE"sv,
		"SMSG_ECHO_PARTY_SQUELCH"sv,
		"CMSG_SET_TITLE_SUFFIX"sv,
		"CMSG_SPELLCLICK"sv,
		"SMSG_LOOT_LIST"sv,
		"CMSG_GM_CHARACTER_RESTORE"sv,
		"CMSG_GM_CHARACTER_SAVE"sv,
		"SMSG_VOICESESSION_FULL"sv,
		"MSG_GUILD_PERMISSIONS"sv,
		"MSG_GUILD_BANK_MONEY_WITHDRAWN"sv,
		"MSG_GUILD_EVENT_LOG_QUERY"sv,
		"CMSG_MAELSTROM_RENAME_GUILD"sv,
		"CMSG_GET_MIRRORIMAGE_DATA"sv,
		"SMSG_MIRRORIMAGE_DATA"sv,
		"SMSG_FORCE_DISPLAY_UPDATE"sv,
		"SMSG_SPELL_CHANCE_RESIST_PUSHBACK"sv,
		"CMSG_IGNORE_DIMINISHING_RETURNS_CHEAT"sv,
		"SMSG_IGNORE_DIMINISHING_RETURNS_CHEAT"sv,
		"CMSG_KEEP_ALIVE"sv,
		"SMSG_RAID_READY_CHECK_ERROR"sv,
		"CMSG_OPT_OUT_OF_LOOT"sv,
		"MSG_QUERY_GUILD_BANK_TEXT"sv,
		"CMSG_SET_GUILD_BANK_TEXT"sv,
		"CMSG_SET_GRANTABLE_LEVELS"sv,
		"CMSG_GRANT_LEVEL"sv,
		"CMSG_REFER_A_FRIEND"sv,
		"MSG_GM_CHANGE_ARENA_RATING"sv,
		"CMSG_DECLINE_CHANNEL_INVITE"sv,
		"SMSG_GROUPACTION_THROTTLED"sv,
		"SMSG_OVERRIDE_LIGHT"sv,
		"SMSG_TOTEM_CREATED"sv,
		"CMSG_TOTEM_DESTROYED"sv,
		"CMSG_EXPIRE_RAID_INSTANCE"sv,
		"CMSG_NO_SPELL_VARIANCE"sv,
		"CMSG_QUESTGIVER_STATUS_MULTIPLE_QUERY"sv,
		"SMSG_QUESTGIVER_STATUS_MULTIPLE"sv,
		"CMSG_SET_PLAYER_DECLINED_NAMES"sv,
		"SMSG_SET_PLAYER_DECLINED_NAMES_RESULT"sv,
		"CMSG_QUERY_SERVER_BUCK_DATA"sv,
		"CMSG_CLEAR_SERVER_BUCK_DATA"sv,
		"SMSG_SERVER_BUCK_DATA"sv,
		"SMSG_SEND_UNLEARN_SPELLS"sv,
		"SMSG_PROPOSE_LEVEL_GRANT"sv,
		"CMSG_ACCEPT_LEVEL_GRANT"sv,
		"SMSG_REFER_A_FRIEND_FAILURE"sv,
		"SMSG_SPLINE_MOVE_SET_FLYING"sv,
		"SMSG_SPLINE_MOVE_UNSET_FLYING"sv,
		"SMSG_SUMMON_CANCEL"sv,
		"CMSG_CHANGE_PERSONAL_ARENA_RATING"sv,
		"CMSG_ALTER_APPEARANCE"sv,
		"SMSG_ENABLE_BARBER_SHOP"sv,
		"SMSG_BARBER_SHOP_RESULT"sv,
		"CMSG_CALENDAR_GET_CALENDAR"sv,
		"CMSG_CALENDAR_GET_EVENT"sv,
		"CMSG_CALENDAR_GUILD_FILTER"sv,
		"CMSG_CALENDAR_ARENA_TEAM"sv,
		"CMSG_CALENDAR_ADD_EVENT"sv,
		"CMSG_CALENDAR_UPDATE_EVENT"sv,
		"CMSG_CALENDAR_REMOVE_EVENT"sv,
		"CMSG_CALENDAR_COPY_EVENT"sv,
		"CMSG_CALENDAR_EVENT_INVITE"sv,
		"CMSG_CALENDAR_EVENT_RSVP"sv,
		"CMSG_CALENDAR_EVENT_REMOVE_INVITE"sv,
		"CMSG_CALENDAR_EVENT_STATUS"sv,
		"CMSG_CALENDAR_EVENT_MODERATOR_STATUS"sv,
		"SMSG_CALENDAR_SEND_CALENDAR"sv,
		"SMSG_CALENDAR_SEND_EVENT"sv,
		"SMSG_CALENDAR_FILTER_GUILD"sv,
		"SMSG_CALENDAR_ARENA_TEAM"sv,
		"SMSG_CALENDAR_EVENT_INVITE"sv,
		"SMSG_CALENDAR_EVENT_INVITE_REMOVED"sv,
		"SMSG_CALENDAR_EVENT_STATUS"sv,
		"SMSG_CALENDAR_COMMAND_RESULT"sv,
		"SMSG_CALENDAR_RAID_LOCKOUT_ADDED"sv,
		"SMSG_CALENDAR_RAID_LOCKOUT_REMOVED"sv,
		"SMSG_CALENDAR_EVENT_INVITE_ALERT"sv,
		"SMSG_CALENDAR_EVENT_INVITE_REMOVED_ALERT"sv,
		"SMSG_CALENDAR_EVENT_INVITE_STATUS_ALERT"sv,
		"SMSG_CALENDAR_EVENT_REMOVED_ALERT"sv,
		"SMSG_CALENDAR_EVENT_UPDATED_ALERT"sv,
		"SMSG_CALENDAR_EVENT_MODERATOR_STATUS_ALERT"sv,
		"CMSG_CALENDAR_COMPLAIN"sv,
		"CMSG_CALENDAR_GET_NUM_PENDING"sv,
		"SMSG_CALENDAR_SEND_NUM_PENDING"sv,
		"CMSG_SAVE_DANCE"sv,
		"SMSG_NOTIFY_DANCE"sv,
		"CMSG_PLAY_DANCE"sv,
		"SMSG_PLAY_DANCE"sv,
		"CMSG_LOAD_DANCES"sv,
		"CMSG_STOP_DANCE"sv,
		"SMSG_STOP_DANCE"sv,
		"CMSG_SYNC_DANCE"sv,
		"CMSG_DANCE_QUERY"sv,
		"SMSG_DANCE_QUERY_RESPONSE"sv,
		"SMSG_INVALIDATE_DANCE"sv,
		"CMSG_DELETE_DANCE"sv,
		"SMSG_LEARNED_DANCE_MOVES"sv,
		"CMSG_LEARN_DANCE_MOVE"sv,
		"CMSG_UNLEARN_DANCE_MOVE"sv,
		"CMSG_SET_RUNE_COUNT"sv,
		"CMSG_SET_RUNE_COOLDOWN"sv,
		"MSG_MOVE_SET_PITCH_RATE_CHEAT"sv,
		"MSG_MOVE_SET_PITCH_RATE"sv,
		"SMSG_FORCE_PITCH_RATE_CHANGE"sv,
		"CMSG_FORCE_PITCH_RATE_CHANGE_ACK"sv,
		"SMSG_SPLINE_SET_PITCH_RATE"sv,
		"CMSG_CALENDAR_EVENT_INVITE_NOTES"sv,
		"SMSG_CALENDAR_EVENT_INVITE_NOTES"sv,
		"SMSG_CALENDAR_EVENT_INVITE_NOTES_ALERT"sv,
		"CMSG_UPDATE_MISSILE_TRAJECTORY"sv,
		"SMSG_UPDATE_ACCOUNT_DATA_COMPLETE"sv,
		"SMSG_TRIGGER_MOVIE"sv,
		"CMSG_COMPLETE_MOVIE"sv,
		"CMSG_SET_GLYPH_SLOT"sv,
		"CMSG_SET_GLYPH"sv,
		"SMSG_ACHIEVEMENT_EARNED"sv,
		"SMSG_DYNAMIC_DROP_ROLL_RESULT"sv,
		"SMSG_CRITERIA_UPDATE"sv,
		"CMSG_QUERY_INSPECT_ACHIEVEMENTS"sv,
		"SMSG_RESPOND_INSPECT_ACHIEVEMENTS"sv,
		"CMSG_DISMISS_CONTROLLED_VEHICLE"sv,
		"CMSG_COMPLETE_ACHIEVEMENT_CHEAT"sv,
		"SMSG_QUESTUPDATE_ADD_PVP_KILL"sv,
		"CMSG_SET_CRITERIA_CHEAT"sv,
		"SMSG_CALENDAR_RAID_LOCKOUT_UPDATED"sv,
		"CMSG_UNITANIMTIER_CHEAT"sv,
		"CMSG_CHAR_CUSTOMIZE"sv,
		"SMSG_CHAR_CUSTOMIZE"sv,
		"SMSG_PET_RENAMEABLE"sv,
		"CMSG_REQUEST_VEHICLE_EXIT"sv,
		"CMSG_REQUEST_VEHICLE_PREV_SEAT"sv,
		"CMSG_REQUEST_VEHICLE_NEXT_SEAT"sv,
		"CMSG_REQUEST_VEHICLE_SWITCH_SEAT"sv,
		"CMSG_PET_LEARN_TALENT"sv,
		"CMSG_PET_UNLEARN_TALENTS"sv,
		"SMSG_SET_PHASE_SHIFT"sv,
		"SMSG_ALL_ACHIEVEMENT_DATA"sv,
		"CMSG_FORCE_SAY_CHEAT"sv,
		"SMSG_HEALTH_UPDATE"sv,
		"SMSG_POWER_UPDATE"sv,
		"CMSG_GAMEOBJ_REPORT_USE"sv,
		"SMSG_HIGHEST_THREAT_UPDATE"sv,
		"SMSG_THREAT_UPDATE"sv,
		"SMSG_THREAT_REMOVE"sv,
		"SMSG_THREAT_CLEAR"sv,
		"SMSG_CONVERT_RUNE"sv,
		"SMSG_RESYNC_RUNES"sv,
		"SMSG_ADD_RUNE_POWER"sv,
		"CMSG_START_QUEST"sv,
		"CMSG_REMOVE_GLYPH"sv,
		"CMSG_DUMP_OBJECTS"sv,
		"SMSG_DUMP_OBJECTS_DATA"sv,
		"CMSG_DISMISS_CRITTER"sv,
		"SMSG_NOTIFY_DEST_LOC_SPELL_CAST"sv,
		"CMSG_AUCTION_LIST_PENDING_SALES"sv,
		"SMSG_AUCTION_LIST_PENDING_SALES"sv,
		"SMSG_MODIFY_COOLDOWN"sv,
		"SMSG_PET_UPDATE_COMBO_POINTS"sv,
		"CMSG_ENABLETAXI"sv,
		"SMSG_PRE_RESURRECT"sv,
		"SMSG_AURA_UPDATE_ALL"sv,
		"SMSG_AURA_UPDATE"sv,
		"CMSG_FLOOD_GRACE_CHEAT"sv,
		"SMSG_SERVER_FIRST_ACHIEVEMENT"sv,
		"SMSG_PET_LEARNED_SPELL"sv,
		"SMSG_PET_REMOVED_SPELL"sv,
		"CMSG_CHANGE_SEATS_ON_CONTROLLED_VEHICLE"sv,
		"CMSG_HEARTH_AND_RESURRECT"sv,
		"SMSG_ON_CANCEL_EXPECTED_RIDE_VEHICLE_AURA"sv,
		"SMSG_CRITERIA_DELETED"sv,
		"SMSG_ACHIEVEMENT_DELETED"sv,
		"CMSG_SERVER_INFO_QUERY"sv,
		"SMSG_SERVER_INFO_RESPONSE"sv,
		"CMSG_CHECK_LOGIN_CRITERIA"sv,
		"SMSG_SERVER_BUCK_DATA_START"sv,
		"CMSG_SET_BREATH"sv,
		"CMSG_QUERY_VEHICLE_STATUS"sv,
		"SMSG_BATTLEGROUND_INFO_THROTTLED"sv,
		"SMSG_SET_VEHICLE_REC_ID"sv,
		"CMSG_RIDE_VEHICLE_INTERACT"sv,
		"CMSG_CONTROLLER_EJECT_PASSENGER"sv,
		"SMSG_PET_GUIDS"sv,
		"SMSG_CLIENTCACHE_VERSION"sv,
		"CMSG_CHANGE_GDF_ARENA_RATING"sv,
		"CMSG_SET_ARENA_TEAM_RATING_BY_INDEX"sv,
		"CMSG_SET_ARENA_TEAM_WEEKLY_GAMES"sv,
		"CMSG_SET_ARENA_TEAM_SEASON_GAMES"sv,
		"CMSG_SET_ARENA_MEMBER_WEEKLY_GAMES"sv,
		"CMSG_SET_ARENA_MEMBER_SEASON_GAMES"sv,
		"SMSG_SET_ITEM_PURCHASE_DATA"sv,
		"CMSG_GET_ITEM_PURCHASE_DATA"sv,
		"CMSG_ITEM_PURCHASE_REFUND"sv,
		"SMSG_ITEM_PURCHASE_REFUND_RESULT"sv,
		"CMSG_CORPSE_TRANSPORT_QUERY"sv,
		"SMSG_CORPSE_TRANSPORT_QUERY"sv,
		"CMSG_UNUSED5"sv,
		"CMSG_UNUSED6"sv,
		"CMSG_CALENDAR_EVENT_SIGNUP"sv,
		"SMSG_CALENDAR_CLEAR_PENDING_ACTION"sv,
		"SMSG_LOAD_EQUIPMENT_SET"sv,
		"CMSG_SAVE_EQUIPMENT_SET"sv,
		"CMSG_ON_MISSILE_TRAJECTORY_COLLISION"sv,
		"SMSG_NOTIFY_MISSILE_TRAJECTORY_COLLISION"sv,
		"SMSG_TALENT_UPDATE"sv,
		"CMSG_LEARN_TALENT_GROUP"sv,
		"CMSG_PET_LEARN_TALENT_GROUP"sv,
		"CMSG_SET_ACTIVE_TALENT_GROUP_OBSOLETE"sv,
		"CMSG_GM_GRANT_ACHIEVEMENT"sv,
		"CMSG_GM_REMOVE_ACHIEVEMENT"sv,
		"CMSG_GM_SET_CRITERIA_FOR_PLAYER"sv,
		"SMSG_DESTROY_ARENA_UNIT"sv,
		"SMSG_ARENA_TEAM_CHANGE_FAILED"sv,
		"CMSG_PROFILEDATA_REQUEST"sv,
		"SMSG_PROFILEDATA_RESPONSE"sv,
		"CMSG_START_BATTLEFIELD_CHEAT"sv,
		"CMSG_END_BATTLEFIELD_CHEAT"sv,
		"SMSG_COMPOUND_MOVE"sv,
		"SMSG_MOVE_GRAVITY_DISABLE"sv,
		"CMSG_MOVE_GRAVITY_DISABLE_ACK"sv,
		"SMSG_MOVE_GRAVITY_ENABLE"sv,
		"CMSG_MOVE_GRAVITY_ENABLE_ACK"sv,
		"MSG_MOVE_GRAVITY_CHNG"sv,
		"SMSG_SPLINE_MOVE_GRAVITY_DISABLE"sv,
		"SMSG_SPLINE_MOVE_GRAVITY_ENABLE"sv,
		"CMSG_USE_EQUIPMENT_SET"sv,
		"SMSG_USE_EQUIPMENT_SET_RESULT"sv,
		"CMSG_FORCE_ANIM"sv,
		"SMSG_FORCE_ANIM"sv,
		"CMSG_CHAR_FACTION_CHANGE"sv,
		"SMSG_CHAR_FACTION_CHANGE"sv,
		"CMSG_PVP_QUEUE_STATS_REQUEST"sv,
		"SMSG_PVP_QUEUE_STATS"sv,
		"CMSG_SET_PAID_SERVICE_CHEAT"sv,
		"SMSG_BATTLEFIELD_MANAGER_ENTRY_INVITE"sv,
		"CMSG_BATTLEFIELD_MANAGER_ENTRY_INVITE_RESPONSE"sv,
		"SMSG_BATTLEFIELD_MANAGER_ENTERING"sv,
		"SMSG_BATTLEFIELD_MANAGER_QUEUE_INVITE"sv,
		"CMSG_BATTLEFIELD_MANAGER_QUEUE_INVITE_RESPONSE"sv,
		"CMSG_BATTLEFIELD_MANAGER_QUEUE_REQUEST"sv,
		"SMSG_BATTLEFIELD_MANAGER_QUEUE_REQUEST_RESPONSE"sv,
		"SMSG_BATTLEFIELD_MANAGER_EJECT_PENDING"sv,
		"SMSG_BATTLEFIELD_MANAGER_EJECTED"sv,
		"CMSG_BATTLEFIELD_MANAGER_EXIT_REQUEST"sv,
		"SMSG_BATTLEFIELD_MANAGER_STATE_CHANGED"sv,
		"CMSG_BATTLEFIELD_MANAGER_ADVANCE_STATE"sv,
		"CMSG_BATTLEFIELD_MANAGER_SET_NEXT_TRANSITION_TIME"sv,
		"MSG_SET_RAID_DIFFICULTY"sv,
		"CMSG_XPGAIN"sv,
		"SMSG_XPGAIN"sv,
		"SMSG_GMTICKET_RESPONSE_ERROR"sv,
		"SMSG_GMTICKET_GET_RESPONSE"sv,
		"CMSG_GMTICKET_RESOLVE_RESPONSE"sv,
		"SMSG_GMTICKET_RESOLVE_RESPONSE"sv,
		"SMSG_GMTICKET_CREATE_RESPONSE_TICKET"sv,
		"CMSG_GM_CREATE_TICKET_RESPONSE"sv,
		"CMSG_SERVERINFO"sv,
		"SMSG_SERVERINFO"sv,
		"CMSG_UI_TIME_REQUEST"sv,
		"SMSG_UI_TIME"sv,
		"CMSG_CHAR_RACE_CHANGE"sv,
		"MSG_VIEW_PHASE_SHIFT"sv,
		"SMSG_TALENTS_INVOLUNTARILY_RESET"sv,
		"CMSG_DEBUG_SERVER_GEO"sv,
		"SMSG_DEBUG_SERVER_GEO"sv,
		"SMSG_LOOT_UPDATE"sv,
		"UMSG_UPDATE_GROUP_INFO"sv,
		"CMSG_READY_FOR_ACCOUNT_DATA_TIMES"sv,
		"CMSG_QUERY_GET_ALL_QUESTS"sv,
		"SMSG_ALL_QUESTS_COMPLETED"sv,
		"CMSG_GMLAGREPORT_SUBMIT"sv,
		"CMSG_AFK_MONITOR_INFO_REQUEST"sv,
		"SMSG_AFK_MONITOR_INFO_RESPONSE"sv,
		"CMSG_AFK_MONITOR_INFO_CLEAR"sv,
		"SMSG_AREA_TRIGGER_NO_CORPSE"sv,
		"CMSG_GM_NUKE_CHARACTER"sv,
		"CMSG_LOW_LEVEL_RAID"sv,
		"CMSG_LOW_LEVEL_RAID_USER"sv,
		"SMSG_CAMERA_SHAKE"sv,
		"SMSG_SOCKET_GEMS"sv,
		"CMSG_SET_CHARACTER_MODEL"sv,
		"SMSG_CONNECT_TO"sv,
		"CMSG_CONNECT_TO_FAILED"sv,
		"SMSG_SUSPEND_COMMS"sv,
		"CMSG_SUSPEND_COMMS_ACK"sv,
		"SMSG_RESUME_COMMS"sv,
		"CMSG_AUTH_CONTINUED_SESSION"sv,
		"CMSG_DROP_NEW_CONNECTION"sv,
		"SMSG_SEND_ALL_COMBAT_LOG"sv,
		"SMSG_OPEN_LFG_DUNGEON_FINDER"sv,
		"SMSG_MOVE_SET_COLLISION_HGT"sv,
		"CMSG_MOVE_SET_COLLISION_HGT_ACK"sv,
		"MSG_MOVE_SET_COLLISION_HGT"sv,
		"CMSG_CLEAR_RANDOM_BG_WIN_TIME"sv,
		"CMSG_CLEAR_HOLIDAY_BG_WIN_TIME"sv,
		"CMSG_COMMENTATOR_SKIRMISH_QUEUE_COMMAND"sv,
		"SMSG_COMMENTATOR_SKIRMISH_QUEUE_RESULT1"sv,
		"SMSG_COMMENTATOR_SKIRMISH_QUEUE_RESULT2"sv,
		"SMSG_COMPRESSED_UNKNOWN_1310"sv,
		"NUM_MSG_TYPES"sv,
	};

	constexpr std::string_view to_string_view(const opcode op)
	{
		return opcode_names[static_cast<unsigned int>(op)];
	}

	inline std::string to_string(const opcode op)
	{
		return std::string{to_string_view(op)};
	}

	inline std::string to_string(const client op)
	{
		return std::string{to_string_view(static_cast<opcode>(op))};
	}

	inline std::string to_string(const server op)
	{
		return std::string{to_string_view(static_cast<opcode>(op))};
	}
}
